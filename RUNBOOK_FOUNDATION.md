...
|---|---|---|
| 0.1 | **Pool source-set membership — whisky market.** `deploy/optimizer_pool.json` says the pool is fed by "exactly two inflows: the auto-launch fees and x402Core. NOTHING ELSE"; `specs/SPEC_PossessioWhiskyMarket.md` §6 + the TODO parked item say the whisky market address must be in the immutable set AT pool deploy (mine salt → address first) — §6 itself allows the alternative "joins a later pool deployment." These conflict. | Export `WHISKY_MARKET_ADDR=0x…` (include, §6 path — mine the market salt first) **XOR** `WHISKY_MARKET_EXCLUDED=true` (ratify the 2-source set; whisky joins a future pool redeploy). `DeployPossessioPool.s.sol` refuses on both-or-neither. |
| 0.2 | **`OPERATOR_DEST_ADDR` / `TREASURY_DEST_ADDR`** — the pool's immutable operator + treasury. No ratified mainnet values exist anywhere in the repo. | Name the two addresses. (treasury == operator is an allowed shape per the contract's documented intentional omission; every SOURCE ≠ both is enforced.) |
...
| 0.8 | **Deployer key + nonce plan** (§II-1). The factory address is predicted from `(deployer, nonce)` — the plan below assumes ONE key sends the three foundation txs back-to-back. Any deviation (any extra tx from that key mid-sequence) shifts the nonce and bricks the wiring. | Confirm the key and that it goes quiet except for this sequence. |
