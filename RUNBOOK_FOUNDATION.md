...
|---|---|---|
| 0.1 | **Pool source-set membership — whisky market.** ~~Conflict between optimizer_pool.json's former two-source wording and whisky spec §6.~~ **RATIFIED 2026-07-14 (Architect): whisky IS in the pool set** — "it is in the pool set. we already worked it out." (`optimizer_pool.json` `_business_model` now names the three-source set directly — P-8 doc-drift fix, 2026-07-15.) **🚨 TEST GAP (verified 2026-07-16): the whisky CONTRACT has ZERO test coverage — not adversarial, not happy-path (the only `whisky` refs in `test/` are a mock `makeAddr` EOA in `PossessioPoolFork.t.sol` testing the POOL, not the market).** | Mine the market's CREATE3 salt (§II), export `WHISKY_MARKET_ADDR=0x…`. The XOR guard in `DeployPossessioPool.s.sol` stands as the mechanical enforcement. **HARD RULE: authorizing whisky's *address* in the pool source set is fine (no code lives there until it deploys — the x402Core/factory authorize-at-construction pattern), but the whisky CONTRACT must NOT be deployed to that address / wired live until it clears its own gauntlet + DoD + the diverse council audit.** Architect's call at broadcast: authorize-the-address-only (deploy the contract later, post-audit) OR `WHISKY_MARKET_EXCLUDED=true` (pool `authorizedSourceCount==2`; note the source set is IMMUTABLE, so exclusion means this pool can never carry whisky — a future pool version would). Either way the untested contract never becomes a live source. |
