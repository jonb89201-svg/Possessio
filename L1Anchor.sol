 ...
 *
 *   ARCHITECTURAL RATIFICATIONS (Architect, May 9, 2026)
 *   ----------------------------------------------------
 *   1. Symmetry Guard:    Stateful pattern (v1 PLATE.sol carried forward).
 *                         Stored bool depegPaused, set on threshold crossing,
 *                         oracle failures preserve last-known state via try/catch.
 ...
 *                         be caged from saving their own capital.
...
contract L1Anchor is ReentrancyGuard {
    ...
    ///         Set/cleared by _checkSymmetry() on threshold crossings.
     ...
     *
     *         Pattern carried forward from PLATE.sol v1 (battle-tested across
     *         testnet, mainnet, and 20 production transactions, four-model
     *         co-authored).
     *
     *         Behavior:
     *         - Reads cbETH/ETH price from Chainlink, wrapped in try/catch.
     *         - On oracle failure, preserves last-known depegPaused state
     *           (graceful degradation; merchant routing continues to be gated
     *           by last known good state, withdrawals still flow).
     *         - On staleness >ORACLE_STALE (3600s), preserves last-known state.
     *         - On price < threshold + not paused → set paused, emit event.
     *         - On price ≥ threshold + paused → clear paused, emit event.
     */
    function _checkSymmetry() internal {
        try CBETH_ETH_ORACLE.latestRoundData() returns (
            ...
            // Oracle staleness check; mirrors PLATE.sol v1 3600s threshold.
            // forge-lint: disable-next-line(block-timestamp)
            if (block.timestamp - updatedAt > ORACLE_STALE) return;
            ...
            } else if (priceUint >= threshold && depegPaused) {
                depegPaused = false;
                emit SymmetryGuardResumed(block.timestamp);
            }
        } catch {
            // Oracle failure: preserve last-known state. Merchant routing
            // continues to be gated by last-known depegPaused flag.
            // Withdrawals are unaffected (per Decision 2 ratification).
        }
    }

    /**
     * @notice Explicit merchant entry point to clear the depeg latch when
     *         the oracle has recovered. Triggers re-evaluation of current
     *         oracle state without committing capital to a route call.
     *
     ...
     *
     *         No additional event emission here — _checkSymmetry() already
     *         emits SymmetryGuardResumed when the latch transitions from
     *         paused to unpaused.
     *
     *         Adversarial finding by Grok (Council seat 3); ratified by
     *         Architect on May 9, 2026.
     */
     ...
     *
     * @return cbEthReturned Total cbETH sent to merchant
     * @return usdcReturned  Total USDC sent to merchant
     */
    function emergencyUnwind() external onlyMerchantOwner nonReentrant returns (uint256 cbEthReturned, uint256 usdcReturned) {
        // Best-effort MAVAN unstake — wrapped in try/catch so a MAVAN-side
        // failure does not block the Morpho leg or the final sweep.
        ...
        if (currentMavanShares > 0) {
            try IMAVANEntry(MAVAN_ENTRY).unstake(currentMavanShares) {
                // intentionally empty; balance check below is authoritative
            } catch { /* MAVAN failure does not block other legs */ }
        }

        // Best-effort Morpho redeem — same pattern.
        ...
        // Sweep all cbETH and USDC the contract holds (whether returned just now
        // or held from prior bridges) to the merchant.
        cbEthReturned = CBETH.balanceOf(address(this));
        usdcReturned  = USDC.balanceOf(address(this));

        // Reset principal tracking since position is fully unwound.
        mavanStakedPrincipal = 0;
        morphoDepositedPrincipal = 0;
