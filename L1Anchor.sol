...
contract L1Anchor is ReentrancyGuard {
    ...
    /**
     * @notice External entry point for the Symmetry Guard check. Permits
     *         the oracle evaluation to run in its own transaction so the
     *         latch state persists across subsequent calls.
     *
     *         Two reasons this exists:
     *
     *         1. Merchant UX: closes the gap where the only way to test
     *            current latch state was to attempt a routeToMavan with
     *            funds at risk. Merchant can now poll the symmetry guard
     *            without committing capital.
     *
     *         2. Architectural: when _checkSymmetry() runs inside
     *            routeToMavan and the route then reverts (because the
     *            check latched paused), all state changes within that
     *            transaction roll back per EVM semantics — including the
     *            depegPaused=true write. The external wrapper lets callers
     *            commit the latch in a separate transaction that does not
     *            revert, so the persisted state survives.
     *
     *         Permissionless by design. The worst case from any caller
     *         invoking this function is the contract correctly observing
     *         a bad oracle and latching itself paused — which is the
     *         contract's intended safety behavior. No funds move, no auth
     *         granted, no return value.
     *
     *         Pattern lineage: guarded-external + unguarded-internal split,
     *         canonical OpenZeppelin pattern, previously applied to
     *         PossessioPayments.sweep() to resolve the analogous
     *         performUpkeep → sweep double-nonReentrant deadlock.
     */
    function checkSymmetry() external {
        _checkSymmetry();
    }

    /**
     * @notice Internal stateful Symmetry Guard check. Called by routeToMavan()
     *         before any capital movement to MAVAN, and by the external
     *         checkSymmetry() wrapper.
     *
