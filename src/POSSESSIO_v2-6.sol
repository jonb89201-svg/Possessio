 ...
 * ╚═╝     ╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝
 *
 * POSSESSIO PROTOCOL — v2.6.1 (STEEL + Chainlink Automation + Aerodrome cbETH route)
...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step, AutomationCompatibleInterface {
    ...
    function routeETH() external nonReentrant notPaused {
        ...
        try this._routeETHDispatch(secret) {
    ...
    ///         them directly.
    function _routeETHDispatch(uint256 secret) external {
        if (msg.sender != address(this)) revert InvalidAddress();
        _routeETH(secret);
    }

    /// @notice v2.6 — Companion dispatcher for the harvest wrapper.
    function _harvestRewardsDispatch(uint256 secret) external {
        if (msg.sender != address(this)) revert InvalidAddress();
    ...
    function _swapETHToDAI(uint256 ethAmt) internal {
        ...
        try CHAINLINK_DAI_ETH.latestRoundData() returns (
            ...
            // DAI/ETH is 8-decimal price feed; ORACLE_STALE_8DEC = 3600s.
            // Validator's ~12s manipulation window is immaterial vs that bound.
    ...
    function _deployToStaking(uint256 ethAmt) internal {
        ...
        // ─── Oracle validation: compute minCbETH for slippage guard ──────
        // Chainlink cbETH/ETH is 18-decimal exchange-rate feed.
        // Formula: expectedCbETH = (ethAmt * 1e18) / cbEthEthRate
        // Where cbEthEthRate is the answer (1 cbETH = N ETH, scaled 1e18).
        uint256 minCbETH = 0;
        try CHAINLINK_CBETH_ETH.latestRoundData() returns (
            ...
            if (valid) {
                // cbEthRate > 0 validated above; positive int256 always fits in uint256.
                // forge-lint: disable-next-line(unsafe-typecast)
                uint256 expectedCbETH = (ethAmt * 1e18) / uint256(cbEthRate);
                minCbETH = (expectedCbETH * SLIPPAGE) / 100;
            }
        } catch {}

        if (minCbETH == 0) {
            // Oracle failure — retain ETH for next routing cycle, don't blind swap.
    ...
    function harvestRewards() external nonReentrant notPaused {
        ...
        try this._harvestRewardsDispatch(secret) {
    ...
    function _harvestRewards(uint256 secret) internal {
        ...
        // ─── Oracle validation: compute minWETH for slippage guard ─────────
        // Same staleness window and decimal handling as _deployToStaking.
        // Inverse formula: expectedWETH = (rewardsCbETH * cbEthRate) / 1e18.
        uint256 minWETH = 0;
        try CHAINLINK_CBETH_ETH.latestRoundData() returns (
            uint80 roundId,
            int256 cbEthRate,
        ...
        // No blind swap on harvest. If oracle is unhealthy, revert and try next cycle.
