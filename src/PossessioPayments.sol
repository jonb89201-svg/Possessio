...
contract PossessioPayments is AccessControl, ReentrancyGuard {
    ...
    function sweep(
        ...
        // Compute current total ETH-equivalent across ALL held LSTs.
        // This reflects actual capital, not historical deposits — so threshold
        // detection accounts for LST rate fluctuations (cbETH/rETH yield, market drift).
    ...
    function _computeCurrentEthEquivalent() internal view returns (uint256) {
        ...
        if (cbBal == 0 && rBal == 0) return 0;

        uint256 cbEth = cbBal > 0 ? LST_RATES.cbEthToEth(cbBal) : 0;
        uint256 rEth  = rBal  > 0 ? LST_RATES.rEthToEth(rBal)   : 0;
        return cbEth + rEth;
    }
