...
contract PossessioPayments is AccessControl, ReentrancyGuard {
    ...
    modifier tlPassed(bytes32 id) {
        ...
        // forge-lint: disable-next-line block-timestamp — Timelock comparison against 48h TIMELOCK; validator's ~12s manipulation window is immaterial.
    ...
    function sweep(
        ...
        // forge-lint: disable-next-line block-timestamp — SWEEP_DELAY is 24 hours; validator's ~12s manipulation window is immaterial against 86,400s.
    ...
    function executeDailyLimitIncrease() external onlyRole(OWNER_ROLE) {
        ...
        // forge-lint: disable-next-line block-timestamp — LIMIT_DELAY is 24 hours; validator's ~12s manipulation window is immaterial against 86,400s.
    ...
    function _rollWindowIfNeeded() internal {
        // forge-lint: disable-next-line block-timestamp — WINDOW_SIZE is 24 hours; validator's ~12s manipulation window is immaterial against 86,400s.
        ...
        external onlyRole(OWNER_ROLE) nonReentrant
    {
        ...
        // forge-lint: disable-next-line block-timestamp — EMERGENCY_DELAY is 7 days; validator's ~12s manipulation window is immaterial against 604,800s.
        if (block.timestamp < q.executeAfter) revert TimelockNotPassed();
    ...
    function _validateOracle() internal view {
        ...
        // forge-lint: disable-next-line block-timestamp — Oracle staleness check at 3600s ORACLE_STALE; validator's ~12s manipulation window is immaterial.
    ...
    function emergencyReady(address token) external view returns (bool) {
        ...
        // forge-lint: disable-next-line block-timestamp — Comparison against 7-day EMERGENCY_DELAY queue time; validator's ~12s manipulation window is immaterial.
