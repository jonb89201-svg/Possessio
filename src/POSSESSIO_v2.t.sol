...
import "../src/POSSESSIO_v2-6-1.sol";
...
contract POSSESSIOv2Test is Test {
    ...
    function test_XLink_DispatchWrapperRejectsExternalCaller() public {
        // _routeETHDispatch is external but gated by msg.sender == address(this).
        // External caller (including TREASURY) must be rejected with InvalidAddress.
        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        hook._routeETHDispatch(1);  // arbitrary secret; gating fires first

        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        vm.prank(TREASURY);
        hook._routeETHDispatch(1);
    }

    function test_XLink_HarvestDispatchRejectsExternalCaller() public {
        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        hook._harvestRewardsDispatch(1);
