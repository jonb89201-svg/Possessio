...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step, AutomationCompatibleInterface {
    ...
    uint256 public constant DAI_BOOT_PCT  = 20;      // 20% of 75% → ETH reserve slice (stays in-contract; was DAI)
    ...
    function _routeETH(uint256 secret) internal {
        ...
        // Staking allocation preserved at its prior proportion (80% of treasury =
        // treasury minus the former 20% DAI-boot slice). The remaining ETH
        // (toTreasury - toStaking) stays in-contract as the ETH reserve.
        uint256 toStaking  = (toTreasury * (100 - DAI_BOOT_PCT)) / 100;
        ...
        // ETHRouted: toDAI is now always 0 (leg removed); the ETH reserve slice
        // is implicit (total - toLP - toStaking remains in-contract for Treasury).
