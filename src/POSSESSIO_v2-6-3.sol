...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step, AutomationCompatibleInterface, EIP712 {
    ...
    function executeInvent(
        ...
        // F3 AMOUNT-BINDING (council 2026-07-20): the approved hash MUST commit to
        // the executed amount + metadata + this deployment. Without this, the
        // council approves an opaque bytes32 and the Treasury could execute any
        // amount against it — a record-integrity gap (not a drain: funds still go
        // to TREASURY_SAFE, onlyTreasury). abi.encode (not encodePacked) is
        // collision-safe as fields grow; address(this) makes the hash
        // self-describing about which deployment it authorizes. The canonical
        // formula lives ONLY here and in the F4 publisher — a preimage that fails
        // to recompute against this line is a bug or a lie, and either way visible.
        if (proposalHash != keccak256(abi.encode(address(this), amount, metadata))) {
