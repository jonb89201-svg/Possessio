...
contract PossessioPayments is AccessControl, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    constructor(
        ...
        if (aeroRouter_   == address(0)) revert InvalidAddress();
        if (morphoVault_  == address(0)) revert InvalidAddress();
        ...
        // ─── Decimal-class validation (v2.3 defense-in-depth) ───
        ...
        // terminal-verified for cbETH/ETH at 0x806b4Ac0…440b on 2026-05-23.
        if (IChainlinkFeed(chainlink_).decimals() != 18) revert InvalidAddress();
        if (IChainlinkFeed(chainlinkDai_).decimals() != 8) revert InvalidAddress();
        ...
        AERO_ROUTER   = IAerodromeSlipstreamRouter(aeroRouter_);
        MORPHO_VAULT  = IMorphoVault(morphoVault_);
