...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step {
    ...
    error MismatchedETH();
    error RescueBlocked();
    error FeeTooLarge();       // feeETH exceeds int128 max — swap size unreachable in practice
    ...
    function getHookPermissions() external pure returns (
        bool hookBeforeInitialize, bool hookAfterInitialize,
        bool hookBeforeAddLiquidity, bool hookAfterAddLiquidity,
        bool hookBeforeRemoveLiquidity, bool hookAfterRemoveLiquidity,
        bool hookBeforeSwap, bool hookAfterSwap,
        bool hookBeforeDonate, bool hookAfterDonate,
        bool hookBeforeSwapReturnDelta, bool hookAfterSwapReturnDelta,
        bool hookAfterAddLiquidityReturnDelta, bool hookAfterRemoveLiquidityReturnDelta
    ) {
        return (
            false, false,
    ...
    function beforeSwap(
        ...
        // Bounds check: feeETH must fit in int128 before delta packing.
        // int128 max ≈ 170 ETH. At 2% fee this requires a single swap of ~8,500 ETH
        // to overflow — unreachable in practice, but enforced for correctness.
        if (feeETH > uint256(uint128(type(int128).max))) revert FeeTooLarge();
        ...
        if (isETHSpecified) {
            ...
            // casting to int128 is safe: feeETH <= type(int128).max enforced above
