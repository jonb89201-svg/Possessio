...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step, AutomationCompatibleInterface {
    ...
    error NotSelf();                   // internal-only entrypoint called externally
    ...
    /// @notice Gates internal-only external entrypoints (dispatchers) to self-calls.
    modifier onlySelf() {
    ...
    function routeETH() external nonReentrant notPaused {
        ...
        //   2. address(this)       — internal dispatcher path; contract would pay itself.
    ...
    /// @notice External-only dispatcher from performUpkeep into routeETH.
    ///         this.routeETH() sets msg.sender to address(this), which triggers
    ///         the non-Treasury permissionless branch in routeETH — applying its
    ///         canonical threshold/cooldown guards. Reward exemption catches
    ///         address(this) per IV.2.
