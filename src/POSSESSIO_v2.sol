...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step {
    ...
    modifier tlPassed(bytes32 id) {
        ...
        // forge-lint: disable-next-line block-timestamp — Timelock comparison is multi-hour scale; validator's ~12s manipulation window is immaterial.
    ...
    function routeETH() external nonReentrant notPaused {
        ...
        if (!isTreasury) {
            ...
            // forge-lint: disable-next-line block-timestamp — ROUTE_COOLDOWN is 6 hours; validator's ~12s manipulation window is immaterial against 21,600s.
    ...
    function _swapETHToDAI(uint256 ethAmt) internal {
        ...
        try CHAINLINK_DAI_ETH.latestRoundData() returns (
            ...
            // forge-lint: disable-next-line block-timestamp — Oracle staleness check at 3600s threshold; validator's ~12s manipulation window is immaterial.
            ...
            if (valid) {
                // forge-lint: disable-next-line unsafe-typecast — daiEthPrice > 0 validated on line above; positive int256 always fits in uint256.
    ...
    function _checkDepeg() internal {
        try CHAINLINK_CBETH_ETH.latestRoundData() returns (
            ...
            if (answer < DEPEG_THRESH && !cbETHPaused) {
                cbETHPaused = true;
                // forge-lint: disable-next-line unsafe-typecast — answer ∈ (0, DEPEG_THRESH=97e6) validated above; (1e8 - answer) ∈ (3e6, 1e8) is always positive, fits uint256.
        ...
        external onlyCouncilMember savNotPaused notSlashed
    {
        ...
        p.approvals = 0;
        // forge-lint: disable-next-line block-timestamp — Expiry computed on a 30-day window; validator's ~12s manipulation window is immaterial.
        ...
        external onlyCouncilMember savNotPaused notSlashed
    {
        ...
        // forge-lint: disable-next-line block-timestamp — Expiry compared against a 30-day window; validator's ~12s manipulation window is immaterial.
