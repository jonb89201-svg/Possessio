...
contract AutomationInvariants is Test {
    ...
    address constant TREASURY  = address(0xT1);
    ...
    function test_ForwarderUpdate_RequiresTimelock() public {
        bytes32 id;
        vm.prank(TREASURY);
        id = hook.queueForwarderUpdate(address(0xNEW));
    ...
    function test_ForwarderUpdate_SucceedsAfterTimelock() public {
        ...
        assertEq(hook.automationForwarder(), address(0xNEW), "forwarder not updated after timelock");
    }
...
contract PaymentsAutomationInvariants is Test {
    ...
    address constant OWNER     = address(0xO1);
    address constant OPERATOR  = address(0xO2);
    address constant ATTACKER  = address(0xBAD);
