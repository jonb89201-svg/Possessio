...
contract PLATETest is Test {
    ...
    function setUp() public {
        ...
        // (clDAI was created before warp — must update to avoid stale feed)
    ...
    function test_Fee_CollectedOnSwapFromPair() public {
        ...
        require(plate.transfer(TREASURY, amount), "PLATE.t.sol:385 transfer must succeed");
        vm.prank(TREASURY);
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:387 transfer must succeed");
        ...
        require(plate.transfer(USER, amount / 2), "PLATE.t.sol:396 transfer must succeed");

        uint256 expectedFee = amount * 200 / 2 / 10_000;
    ...
    function test_Fee_CollectedOnSwapToPair() public {
        uint256 amount = 10_000 * 1e18;
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:405 transfer must succeed");
    ...
    function test_Fee_ZeroOnWalletTransfer() public {
        require(plate.transfer(USER, 1_000 * 1e18), "PLATE.t.sol:412 transfer must succeed");
    ...
    function test_Fee_ZeroForExcludedAddress() public {
        ...
        require(plate.transfer(TREASURY, 1_000 * 1e18), "PLATE.t.sol:419 transfer must succeed");
        vm.prank(TREASURY);
        require(plate.transfer(address(pool), 1_000 * 1e18), "PLATE.t.sol:421 transfer must succeed");
    ...
    function test_Fee_PendingFeesIncrements() public {
        ...
        require(plate.transfer(address(pool), amt1), "PLATE.t.sol:428 transfer must succeed");
        require(plate.transfer(address(pool), amt2), "PLATE.t.sol:429 transfer must succeed");
    ...
    function test_Fee_ExactMath() public {
        uint256 amount = 10_000 * 1e18;
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:437 transfer must succeed");
    ...
    function test_Fee_EventEmittedCorrectly() public {
        ...
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:453 transfer must succeed");
    }

    function test_Fee_PausedRoutingSkipsFeeCollection() public {
        plate.pauseRouting();
        uint256 amount = 1_000 * 1e18;
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:459 transfer must succeed");
    ...
    function test_Swap_RevertsBelowMinBatch() public {
        ...
        require(plate.transfer(address(pool), 1 * 1e18), "PLATE.t.sol:483 transfer must succeed");
        vm.warp(block.timestamp + 25 hours);
    ...
    function test_Swap_BootstrapDoesNotRevertWithValidRefPrice() public {
        ...
        require(freshPlate.transfer(address(pool), needed), "PLATE.t.sol:519 transfer must succeed");
        ...
        require(freshPlate2.transfer(address(pool), needed2), "PLATE.t.sol:537 transfer must succeed");
    ...
    function test_Swap_SecondSwapSucceedsAfterDelay() public {
        ...
        _seedFees(plate.minSwapBatch() + 1);
        // Warp well past the 24h SWAP_DELAY boundary with explicit margin
        vm.warp(block.timestamp + 25 hours + 1);
        // Refresh mock pool ticks so post-bootstrap TWAP path has fresh observation
        pool.setTicks(0, int56(-497383200));
        router.setEthReturn(1 ether);
    ...
    function test_DAISwap_FallsBackWhenFeedStale() public {
        ...
        // After Finding 6 patch: stale oracle routes ETH to Treasury
        // instead of swapping with minDAI = 1 (no slippage protection)
        plate.routeETH();
        // DAI reserve unchanged — swap aborted
        assertEq(plate.daiReserve(), 0, "Stale oracle: DAI reserve must not increase");
        // ETH routed to Treasury fallback
        assertGt(TREASURY.balance, treasuryBefore, "Stale oracle: ETH must route to Treasury");
    }
    ...
    function test_DAISwap_FallsBackWhenFeedReverts() public {
        ...
        vm.deal(address(plate), 1 ether);
        uint256 treasuryBefore = TREASURY.balance;
        // After Finding 6 patch: reverted oracle routes ETH to Treasury
        // instead of swapping with minDAI = 1 (no slippage protection)
        plate.routeETH();
        // DAI reserve unchanged — swap aborted
        assertEq(plate.daiReserve(), 0, "Oracle revert: DAI reserve must not increase");
        // ETH routed to Treasury fallback
        assertGt(TREASURY.balance, treasuryBefore, "Oracle revert: ETH must route to Treasury");
    }
    ...
    function test_Staking_CbETHReceives40Percent() public {
        uint256 total     = 10 ether;
    ...
    function test_Staking_RETHReceives60Percent() public {
        ...
        uint256 toDAI     = toT * 20 / 100;
        uint256 toStaking = toT - toDAI;
        uint256 expectedRETH = toStaking * 60 / 100;

        plate.routeETH();

        assertApproxEqAbs(rETH.balanceOf(address(plate)), expectedRETH, 0.01 ether,
            "rETH must receive exactly 60% of staking allocation");
    ...
    function test_Staking_WhenCbETHPaused_RedirectsToRETH() public {
        ...
        // cbETH allocation redirects to rETH — rETH receives 100% of staking
        assertApproxEqAbs(rETH.balanceOf(address(plate)), toStaking, 0.01 ether,
            "rETH must receive 100% of staking when cbETH is paused");
    }
    ...
    function test_Staking_AllocationsSum100Percent() public {
        ...
        uint256 toDAI     = toT * 20 / 100;
        uint256 toStaking = toT - toDAI;
        uint256 expCbETH  = toStaking * 40 / 100;
        uint256 expRETH   = toStaking * 60 / 100;
        ...
        assertApproxEqAbs(expCbETH + expRETH, toStaking, 0.01 ether,
            "cbETH + rETH must equal total staking allocation");
    ...
    function test_CB_TransfersWorkWhenPaused() public {
        plate.pauseRouting();
        require(plate.transfer(USER, 1_000 * 1e18), "PLATE.t.sol:1187 transfer must succeed");
    ...
    function testFuzz_FeeMath(uint256 amount) public {
        ...
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:1312 transfer must succeed");
    ...
    function testFuzz_FeeConservation(uint256 amount) public {
        ...
        require(plate.transfer(address(pool), amount), "PLATE.t.sol:1326 transfer must succeed");
    ...
    function _seedFees(uint256 target) internal {
        ...
        require(plate.transfer(address(pool), needed), "PLATE.t.sol:1371 transfer must succeed");
    }
    ...
    function _seedRouterPlate(uint256 amount) internal {
        ...
        require(plate.transfer(address(router), seedAmount), "PLATE.t.sol:1384 transfer must succeed");
    ...
    function testFuzz_Invariant_SymmetryGuard(uint256 tickManipulation) public {
        ...
        // Use direct int56 cast safely within verified range; tickManipulation bounded [1, 887272] on line above, well within int56 range.
    ...
    function testFuzz_Invariant_GasDeterminism(uint256 dustRounds) public {
        ...
        for (uint256 i = 0; i < dustRounds; i++) {
            ...
            if (plate.balanceOf(address(this)) >= needed) {
                require(plate.transfer(address(pool), needed / dustRounds + 1), "PLATE.t.sol:1638 transfer must succeed");
            }
        }
