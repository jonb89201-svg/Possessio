...
contract L1AnchorOracleManipulationTests is L1AnchorTestBase {
    function test_oracle_negativePrice_blocksRouting() public {
        ...
        // Commit the latch in its own transaction so the depegPaused write
        // persists past the subsequent reverting route call. Without this,
        // the latch is set inside routeToMavan's transaction and rolled back
        // by the DepegPaused revert per EVM state-rollback semantics.
        ...
        // Latch should be set as a result of the prior checkSymmetry call
    ...
    function test_oracle_zeroPrice_blocksRouting() public {
        ...
        // Commit the latch in its own transaction (see negativePrice rationale)
        anchor.checkSymmetry();
    ...
    function test_oracle_revertingFeed_blocksRouting() public {
        ...
        // Commit the latch in its own transaction (see negativePrice rationale).
        // try/catch in _checkSymmetry latches depegPaused=true on oracle revert
        // per 2026-05-17 ratified one-way latch design.
        anchor.checkSymmetry();
    ...
    function test_oracle_oneWayLatch_holdsAfterRecovery() public {
        // Healthy → depegged → latch committed (separate tx) → call reverts
        // → oracle recovers → call still reverts (one-way latch)
        _seedAnchorWithCbEth(20 ether);
        ...
        // Commit the latch in its own transaction so depegPaused=true persists
        // past the subsequent reverting route call (EVM state-rollback semantics).
        anchor.checkSymmetry();
        assertTrue(anchor.depegPaused());

        // Second call: latch is already set, reverts immediately
        vm.prank(merchant);
        vm.expectRevert(L1Anchor.DepegPaused.selector);
        ...
        // Third call: latch holds independent of oracle state. The one-way
        // latch design means even with the oracle healthy, depegPaused is
        // only cleared by an explicit resetDepeg() call.
        vm.prank(merchant);
        vm.expectRevert(L1Anchor.DepegPaused.selector);
        anchor.routeToMavan(5 ether);
        assertTrue(anchor.depegPaused());
