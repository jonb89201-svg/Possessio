...
Everything in this README can be confirmed by inspection. Read the source. Run `forge test`. Query the chain. 487 tests pass. The contracts are deployed or forge-verified. Run it yourself.
...
**Result (May 2, 2026): 487 tests passed · 0 failed · 0 skipped**

Build verified: `forge build` succeeds with Solc 0.8.34, compiling 112 files with 0 errors. Forge lint surfaces 100 informational warnings (`erc20-unchecked-transfer` in test orchestration, `block-timestamp` in deliberate timelock logic, `unsafe-typecast` in V4 hook math, `divide-before-multiply` precision review). Warnings are documented and addressable; none block compilation or test execution.
...
| `POSSESSIO_v2_Invariants_t.sol` | 17 |
| `SAV.t.sol` | 100 |
