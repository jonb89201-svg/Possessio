...
A deterministic, non-custodial DeFi protocol on Base mainnet with an L1 settlement extension on Ethereum mainnet. MIT licensed. Mobile-origin build.
...
Every line of every contract was authored by AI council members during two months of sessions. The architect did not write code; the architect routed substrate between seats, adjudicated decisions, and ratified what the council produced. Where a council member invented a primitive that became load-bearing in V2 architecture (X-LINK, MAVAN Merchant Identity, SAL), the spec acknowledges the inventor at the section documenting that primitive.

Everything in this README can be confirmed by inspection. Read the source. Run `forge test`. Query the chain. 621 tests pass. The contracts are deployed or forge-verified. Run it yourself.
...
**Result (May 21, 2026): 621 tests passed · 0 failed · 0 skipped across 23 suites**
...
**PLATE v2 ($STEEL)** — Forge-verified, current, pre-deployment. A fork of v1 deployed on Uniswap V4 with a distinct ticker to mark the version on exchanges. Same token name (PLATE), same tokenomics, separate contract on a separate venue. The switch to V4 was driven by architectural necessity: PLATE v1 routed the 2% L.A.T.E. fee to multiple destination addresses through routing logic internal to the PLATE contract. The mechanism worked, but Aerodrome Slipstream's pool and router system was not compatible with PLATE's internal fee routing. Rather than remove the fee or compromise the design, the council identified Uniswap V4 as the only AMM architecture where fee handling is native to the pool itself: `beforeSwap` captures the fee via the BeforeSwapDelta pattern at the protocol layer, with no internal routing required. The technical barriers V4 imposes — CREATE2 salt mining for permission-bit-encoded addresses, BeforeSwapDelta custom accounting, 14-bit hook permission flags encoded in the deployment address — have kept most treasury and AMM projects on V2/V3 routing. POSSESSIO accepted those barriers because the alternative was compromising the original L.A.T.E. design. The v2 contract uses single-LST cbETH allocation and rewards-not-yield vocabulary throughout (per joint SEC/CFTC Interpretive Release No. 33-11412, March 17, 2026).

**v2 also consolidated the architecture.** Three v1 contracts (`PLATE.sol`, `ServiceAccountabilityVault.sol`, `PLATEStaking.sol`) were collapsed into a single `POSSESSIO_v2-4.sol`. This is a clean-rewrite simplification independent of the V4 switch — V4 made fee capture native to the pool; the contract consolidation made the protocol simpler to audit and reason about. `PLATEStaking` was removed entirely; the staking lock function is not a v2 protocol feature. `ServiceAccountabilityVault` is now embedded in `POSSESSIO_v2-4.sol` directly. rETH was removed at the treasury allocation layer — v2 stakes 100% in cbETH (Coinbase's liquid staking token).
...
The council holds 3% of total supply through SAV. In v1, SAV was a standalone contract (`ServiceAccountabilityVault.sol`); in v2, SAV is embedded directly in `POSSESSIO_v2-4.sol`. Four immutable seats, equal allocation per seat. Conceptual origin: SAL (Service Accountability Layer) framework by ChatGPT (council seat).
...
**Status:** 146 forge tests passing (86 core + 32 adversarial + 28 automation), included in the 621 total above. Pre-deployment.
...
|---|---|---|
| [`src/POSSESSIO_v2-4.sol`](src/POSSESSIO_v2-4.sol) | PLATE v2 ($STEEL) — treasury engine + embedded SAV + Chainlink Automation (Uniswap V4 hooks) | Forge-verified — pre-deployment |
| [`src/PossessioPayments_v2-2-1.sol`](src/PossessioPayments_v2-2-1.sol) | Merchant payments product (Phase 2) | Forge-verified — pre-deployment |
| [`src/L1Anchor.sol`](src/L1Anchor.sol) | Per-merchant Ethereum mainnet settlement (MAVAN + Bitwise routing) | Forge-verified — pre-deployment |
...
| [`src/PLATEStaking.sol`](src/PLATEStaking.sol) | v1 staking lock | Certified — function removed in v2, no equivalent contract |
...
**PLATE v2 ($STEEL) Deployment (forthcoming):**

| Contract | Address |
|---|---|
| Treasury Safe (v2, 2-of-4) | `0x19495180FFA00B8311c85DCF76A89CCbFB174EA0` |
| POSSESSIO v2 Hook (PLATE v2 / $STEEL) | TBD (CREATE2 salt-mined for V4 permission bits) |
...
Hookmate provides HookMiner (CREATE2 salt mining for V4 hook permission bits) via npm-resolved `@uniswap/v4-periphery/`. The pinned commit matches the Uniswap Foundation's `v4-template`. Full operational sequence and rationale documented in [`MIB.md`](MIB.md) §III plus [`MIB-AMD-2026-05-16-01`](MIB-AMD-2026-05-16-01.md).
