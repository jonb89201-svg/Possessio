...
A sentinel script in [`script/`](script/), authored by the council to protect its own SAV allocation. Runs before any high-stakes execution: environment integrity, sequencer validation, JIT liquidity detection, tick concentration scanning, oracle divergence checks. TOTP human-in-the-loop verification. Fail-closed throughout.

The notable property is that the council wrote it. The architect calibrated the setup, but the design and authorship are council work. The council protects the council's own allocation. That loop closes the structural commitment.
