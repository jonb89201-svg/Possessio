...
Everything in this README can be confirmed by inspection. Read the source. Run `forge test`. Query the chain. **1068 passed / 0 failed / 1 skipped across 60 suites** (unified offline + Base-mainnet-fork run, verified 2026-07-29; run `forge test` for the live tally). Nine V3 contracts are live on Base mainnet -- every address in [Live Contracts](#live-contracts-base-mainnet) was read back from the chain with `eth_getCode` after deploy, not copied from a broadcast log; the remainder are forge-verified or fork-proven and pre-deployment. Run it yourself.
