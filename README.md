...
Everything in this README can be confirmed by inspection. Read the source. Run `forge test`. Query the chain. **973 passed / 0 failed / 9 skipped across 53 suites** (verified 2026-07-22; run `forge test` for the live tally). Two V3 contracts (LSTExchangeRate, PossessioPayments) are live on Base mainnet; the remainder are forge-verified or fork-proven and pre-deployment. Run it yourself.
...
**Result: 973 passed / 0 failed / 9 skipped (982 total) across 53 suites** -- as of the 2026-07-22 AI-trading-desk build (arc across the build's history: 487 -> 585 -> 621 -> 690 -> 765 -> 778 -> 889 -> 973). The suite is actively extended, so the live `forge test` output is the count of record when this line and the terminal disagree. The suite spans unit, adversarial/gauntlet, invariant, fuzz, ffi, and fork tests; the per-suite breakdown is printed by `forge test` itself. Fork suites skip cleanly when no fork RPC is configured (the 9 skips are fork-gated paths that run and pass live, plus one intentional mock-mode variant). The 2026-07-20 pre-freeze additions: the hook's F3 vote-amount binding + `approveInventBySig` (EIP-712, replay-protected), x402Core surplus->Heart brick-guard, and radar rug-gate calibration. The 2026-07-21/22 additions: the on-chain AI trading desk -- **PossessioFundingVault** (trader's hard-capped closed-loop custody, both never-regress invariants mutation-verified), **PossessioRail** (the swap+custody engine that binds each trade to an AutoTarget intent), and **PossessioAutoTarget** (the intent/target/stop engine) -- each with a unit + adversarial + fork suite, plus `CouncilPreimageRoundTrip` proving the council connector's F3 preimage equals the contract's Solidity `abi.encode` byte-for-byte. All mutation-verified.
