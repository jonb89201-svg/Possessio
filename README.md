...
Everything in this README can be confirmed by inspection. Read the source. Run `forge test`. Query the chain. **1007 passed / 0 failed / 1 skipped across 54 suites** (unified offline + Base-mainnet-fork run, verified 2026-07-25; run `forge test` for the live tally). Nine V3 contracts are live on Base mainnet -- every address in [Live Contracts](#live-contracts-base-mainnet) was read back from the chain with `eth_getCode` after deploy, not copied from a broadcast log; the remainder are forge-verified or fork-proven and pre-deployment. Run it yourself.
...
**Certified baseline -- 2026-07-25, one unified run with both modes on the same line:**

- **With a Base-mainnet fork RPC (QuickNode): `1007 passed / 0 failed / 1 skipped` (1008 total) across 54 suites in 108.64s.** Every fork suite executes for real against live Base mainnet state -- the 55 fork tests among them. The lone skip is one intentional mock-mode-only variant (`test_Sweep_RevertsMorphoNoShares_NEEDS_MOCK_MODE`).
- **Offline, no RPC: `953 passed / 0 failed / 38 skipped`.** The fork-dependent paths skip cleanly and loudly; nothing passes without executing.

Both figures are true and are stated together because neither alone is the whole picture. Passing-count arc across the build's history: 487 -> 585 -> 621 -> 690 -> 765 -> 778 -> 889 -> 973 -> **1007** (the first unified offline + mainnet-fork baseline; earlier figures were offline runs with the fork suites gated). The suite is actively extended, so the live `forge test` output is the count of record when this line and the terminal disagree; it spans unit, adversarial/gauntlet, invariant, fuzz, ffi, and fork tests, and the per-suite breakdown is printed by `forge test` itself.
...
The off-chain organs carry their own suites, all green same-day: radar **35/35**, solana-mcp **11/11**, xtrade **58/58**, and the new **council-signer 22/22** (`node --test` in each directory).
...
| [`mcp/xtrade/`](mcp/xtrade/) | Constitution-gated trading MCP -- RULEBOOK gates (session/rug/entry), $2 per-trade / $20 daily caps enforced server-side, build-only (unsigned payloads; hot path hard-refuses until facts are device-verified) | Build-only by design -- tests 58/58 |
