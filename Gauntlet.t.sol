...
contract PLATEAttackTest is Test {
    ...
    // LPFailed(ethAmt, 3) must emit, ETH stays isolated
    // ============================================================
    function test_Attack_LP_SlippageBreach_ZeroPlateReturned() public {
        // plateReturn = 0 means swap succeeds but transfers nothing
        router.setPlateReturn(0);
        router.setDAIReturn(1000 * 1e18);

        // Do NOT seed router with PLATE — zero return is the condition
        vm.deal(address(plate), 4 ether);

        clDAI.setAnswer(int256(500_000));

        vm.expectEmit(false, false, false, true);
        emit PLATE.LPFailed(1 ether, 3); // LP allocation = 25% of 4 ETH

        plate.routeETH();

        // No PLATE stranded
        assertEq(plate.balanceOf(address(plate)), 0,
            "No PLATE stranded after zero-return swap");

        // V3 isolation: LP ETH (25%) stays in contract
        uint256 expectedLpRemainder = 4 ether * 25 / 100;
        assertApproxEqAbs(address(plate).balance, expectedLpRemainder, 0.01 ether,
            "ETH isolation holds after Code 3 — LP portion retained");
    }

    // ============================================================
    // ATTACK 18: LP CODE 5 — NO VALID PRICE (BOTH SOURCES ZERO)
    // Bootstrap reference price = 0, TWAP returns 0
    // minPlateOut stays 0 — LPFailed(ethAmt, 5) must emit
    // ETH stays isolated, no swap attempted
    // ============================================================
    function test_Attack_LP_NoPriceAvailable() public {
        // Deploy fresh contract with referencePrice = 0
        // This forces bootstrap path with no price anchor
        PLATE zeroPlate = new PLATE(
            address(0x9999),
            address(router),
            address(cbETH),
            address(0),
        ...
        // Set pool with zero ticks — TWAP will return 0
        MockPoolAttack zeroPool = new MockPoolAttack(address(zeroPlate));
        zeroPool.setTicks(0, 0);
        bytes32 id = zeroPlate.queueLPUpdate(address(zeroPool));
        vm.warp(block.timestamp + 48 hours + 1);
        zeroPlate.executeLPUpdate(id, address(zeroPool));

        router.setDAIReturn(1000 * 1e18);
        vm.deal(address(zeroPlate), 4 ether);

        clDAI.setAnswer(int256(500_000));

        vm.expectEmit(false, false, false, true);
        emit PLATE.LPFailed(1 ether, 5);

        zeroPlate.routeETH();

        // No PLATE stranded
        assertEq(zeroPlate.balanceOf(address(zeroPlate)), 0,
            "No PLATE stranded after Code 5");

        // V3 isolation: LP ETH (25%) stays
        uint256 expectedLpRemainder = 4 ether * 25 / 100;
        assertApproxEqAbs(address(zeroPlate).balance, expectedLpRemainder, 0.01 ether,
            "ETH isolation holds after Code 5 — LP portion retained");
    }
