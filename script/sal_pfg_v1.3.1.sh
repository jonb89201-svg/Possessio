#!/bin/bash
# ============================================================
# POSSESSIO SAL: Pre-Flight Guard v1.3.1
# Amendment V Ratified | Mobile Nomad Standard
# ============================================================
# CHANGELOG v1.3.1:
#   Gate 4 officially designated: KINETIC DEPTH ANCHOR (KDA)
#   Nomenclature codified in protocol documentation.
#   Threshold comment corrected: 0.01 ETH is Architect
#   configured value. Comment drift fixed.
#
# CHANGELOG v1.3.0:
#   Gate 4 ACTIVATED — Kinetic Depth Anchor (KDA) via
#   quoteExactInputSingle on Aerodrome Slipstream QuoterV2.
#   Replaces slot0-dependent tick scan. No slot0 required.
#   Quoter: 0x166128B234e7939180371457008B17130F309597
#   tickSpacing: 200 (confirmed WETH/PLATE pool)
#
#   Gate 5 ACTIVATED — Synthetic Price Discovery via
#   reserve ratio delta. No slot0 required.
#
# CHANGELOG v1.2.2:
#   Heartbeat: git blob SHA replaces sha256sum.
#   Integer formatting. Gate 3 reserve integrity.
#   Gate 3b delta. Known limitations documented.
#
# NOTE: v1.2.3 and v1.3.0 were design iterations. v1.3.1
#   deploys from v1.2.2 committed baseline. RATIFIED_COMMIT
#   must point to the committed v1.3.1 script SHA.
# ============================================================
# DEPENDENCIES: cast (Foundry), oathtool, curl, bc, awk, git
#
...
#   MIN_RESERVE_THRESHOLD  — 10000000000000000 (0.01 ETH — Architect configured value)
...
#
# USAGE:
#   set -a && source .env.secret && set +a && bash script/sal_pfg_v1.3.1.sh
# ============================================================
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  POSSESSIO SAL: PRE-FLIGHT GUARD v1.3.1      ║"
...
echo "   Fetching remote blob SHA: script/sal_pfg_v1.3.1.sh..."
REMOTE_SCRIPT_HASH=$(fetch_remote_hash "script/sal_pfg_v1.3.1.sh") || {
...
LOCAL_SCRIPT_HASH=$(git hash-object script/sal_pfg_v1.3.1.sh)
LOCAL_PLATE_HASH=$(git hash-object src/PLATE.sol)

HASH_FAIL=0
if [ "$LOCAL_SCRIPT_HASH" != "$REMOTE_SCRIPT_HASH" ]; then
    echo "❌ HEARTBEAT FAIL: script/sal_pfg_v1.3.1.sh does not match ratified commit"
...
if [[ -z "$QUOTE_RAW" ]]; then
    ...
    AMOUNT_OUT=$(echo "$QUOTE_RAW" | awk -F',' '{print $1}' | tr -d ' ')
