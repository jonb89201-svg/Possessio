...
contract MineHookSalt is Script {
    ...
    function run() external view {
        ...
        address chainlinkDaiUsd = vm.envAddress("CHAINLINK_DAI_USD");
        address chainlinkEthUsd = vm.envAddress("CHAINLINK_ETH_USD");
        address v3Router        = vm.envAddress("V3_ROUTER");

        // V3 -- Aerodrome Slipstream Swap Router (canonical on Base).
        // Address verified May 26, 2026 via multi-source cross-reference:
        //   1. BaseScan nametag: "Aerodrome: SlipStream Swap Router"
        //      https://basescan.org/address/0xbe6d8f0d05cc4be24d5167a3ef062215be6d18a5
        //   2. BaseScan transaction history confirms active router behavior
        //      (WETH wrapping + token swap routing on Base mainnet).
        //   3. Source repo: github.com/aerodrome-finance/slipstream (last
        //      updated Jan 16, 2026 -- actively maintained canonical contracts).
        //
        // Required terminal verification before deploy (Bugcatcher's lane):
        //   cast code 0xBE6D8f0d05cC4be24d5167a3eF062215bE6D18a5 --rpc-url $BASE_RPC
        //     -> must return non-empty bytecode
        //   cast call 0xBE6D8f0d05cC4be24d5167a3eF062215bE6D18a5 "factory()(address)" --rpc-url $BASE_RPC
        //     -> must return Aerodrome CL Factory address
        //   cast call 0xBE6D8f0d05cC4be24d5167a3eF062215bE6D18a5 "WETH9()(address)" --rpc-url $BASE_RPC
        //     -> must return 0x4200000000000000000000000000000000000006 (Base WETH)
        //
        // If all three verifications pass, the address is canonical-verified.
        // If any fails, halt deploy and re-route through council.
        address aeroRouter      = 0xBE6D8f0d05cC4be24d5167a3eF062215bE6D18a5;
        ...
        PossessioHook.DeployParams memory params = PossessioHook.DeployParams({
            ...
            chainlinkDaiUsd: chainlinkDaiUsd,
            chainlinkEthUsd: chainlinkEthUsd,
            v3Router:       v3Router,
            aeroRouter:     aeroRouter,  // V3 -- Aerodrome Slipstream WETH/cbETH venue
            weth:           weth,
            council:        council
        });
