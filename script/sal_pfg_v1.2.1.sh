...
#   Gate 3b: liquidity() reverts on Slipstream — replaced with balanceOf delta.
#   Gate 4/5: slot0 reverts on this pool via all ABI variants.
#   Root cause unknown. Both gates bypass gracefully. Open research item.
# ============================================================
...
#
# USAGE:
#   set -a && source .env.secret && set +a && bash script/sal_pfg_v1.2.1.sh
# ============================================================
...
if [ "$LOCAL_SCRIPT_HASH" != "$REMOTE_SCRIPT_HASH" ]; then
    echo "❌ HEARTBEAT FAIL: script/sal_pfg_v1.2.1.sh does not match ratified commit"
...
# ============================================================
# NOTE: slot0 reverts consistently on this pool via all ABI variants.
# Root cause unknown — pool has 20+ trades, responsive on all other calls.
# Open research item. Gates 4 and 5 both bypass gracefully on revert.
# Correct Slipstream ABI: slot0()(uint160,int24,uint16,uint16,uint16,bool)
# ============================================================
echo ""
echo "[4/5] TICK SCAN: Checking active tick concentration..."

SLOT0_NOW=$(cast call "$POOL_ADDR" \
    "slot0()(uint160,int24,uint16,uint16,uint16,bool)" \
...
if [[ -z "$SLOT0_NOW" || -z "$SLOT0_PRIOR" ]]; then
    echo "⚠️  TICK WARNING: slot0 unavailable on this pool — Gate 4 bypassed"
    echo "   Root cause unknown. Open research item. TOTP gate remains active."
    WARNINGS=$((WARNINGS + 1))
    CURRENT_TICK=""
    PRIOR_TICK=""
    SQRT_PRICE=""
...
if [[ -z "$CURRENT_TICK" ]]; then
    : # slot0 unavailable — already warned above, skip concentration check
elif [ "$BELOW_FLOOR" -eq 1 ]; then
    echo "⚠️  TICK WARNING: Pool balance ${LIQ_NOW} below floor ${MIN_LIQ_FLOOR}"
...
# ============================================================
# Bypassed gracefully if slot0 unavailable (same root cause as Gate 4)
# ============================================================
echo ""
echo "[5/5] ORACLE: Checking PLATE/ETH 10-block price delta..."

if [[ -z "$SQRT_PRICE" || -z "$SQRT_PRIOR" ]]; then
    echo "⚠️  ORACLE WARNING: slot0 unavailable — Gate 5 bypassed"
    echo "   Same root cause as Gate 4. Open research item."
    WARNINGS=$((WARNINGS + 1))
else
    ...
    if [ "$(echo "$PRIOR_PLATE_PER_ETH > 0" | bc)" -eq 1 ]; then
        ...
        echo "⚠️  ORACLE WARNING: Cannot compute prior price — bypassing"
        WARNINGS=$((WARNINGS + 1))
    fi
fi
