...
#   Gate 3b: liquidity() reverts on Slipstream — replaced with
#   balanceOf delta. Same JIT signal, proven ABI on this pool.
# ============================================================
...
#
# FIRST-RUN SETUP:
#   Commit this file, note the commit SHA, set RATIFIED_COMMIT
# ============================================================
...
# ============================================================
# Sub-check A: Actual reserve balances via balanceOf (terminal lock)
#   liquidity()(uint128) returns in-range liquidity only on Slipstream.
#   balanceOf reads reflect true reserves regardless of tick range.
#   Single-sided drain is a JIT signal regardless of the other side.
#
# Sub-check B: 10-block balance delta (veto)
#   liquidity() reverts on Aerodrome Slipstream — confirmed on dry run.
#   balanceOf delta used instead — same JIT signal, proven ABI.
# ============================================================
echo ""
...
# liquidity()(uint128) reverts on Aerodrome Slipstream — confirmed on dry run.
# Replaced with balanceOf delta — same JIT signal, proven ABI on this pool.
# NOTE: 15% threshold is loose for a thin bootstrapping pool (~0.1 ETH depth).
# A single LP position can move balances beyond 15% without JIT activity.
# Revisit and tighten threshold once pool has meaningful depth.
...
# Set LIQ_NOW and LIQ_PRIOR for Gate 4 floor check
LIQ_NOW="$BAL0_RAW"
