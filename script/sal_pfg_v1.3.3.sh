#!/bin/bash
# ============================================================
# POSSESSIO SAL: Pre-Flight Guard v1.3.3
# Amendment V Ratified | Mobile Nomad Standard
# ============================================================
# CHANGELOG v1.3.3:
#   Gate 5 hardening — null check on ORACLE_PRICE_ABS before
#   bc division. Empty Chainlink response no longer feeds into
#   bc causing syntax error. Warns and skips display instead.
#   This was the last known parsing flaw. No logic changes.
#
# CHANGELOG v1.3.2:
#   Cast suffix fix — sed 's/\[.*\]//' on all balance reads.
#   Fixes Gate 3b false positive and Gate 5 empty ratio.
#
# CHANGELOG v1.3.1:
...
#
# NOTE: RATIFIED_COMMIT must point to committed v1.3.3 SHA.
# ============================================================
...
#
# USAGE:
#   set -a && source .env.secret && set +a && bash script/sal_pfg_v1.3.3.sh
# ============================================================
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  POSSESSIO SAL: PRE-FLIGHT GUARD v1.3.3      ║"
...
echo "   Fetching remote blob SHA: script/sal_pfg_v1.3.3.sh..."
REMOTE_SCRIPT_HASH=$(fetch_remote_hash "script/sal_pfg_v1.3.3.sh") || {
...
LOCAL_SCRIPT_HASH=$(git hash-object script/sal_pfg_v1.3.3.sh)
LOCAL_PLATE_HASH=$(git hash-object src/PLATE.sol)

HASH_FAIL=0
if [ "$LOCAL_SCRIPT_HASH" != "$REMOTE_SCRIPT_HASH" ]; then
    echo "❌ HEARTBEAT FAIL: script/sal_pfg_v1.3.3.sh does not match ratified commit"
...
# Logic: Price Impact beyond threshold = hollow or manipulated.
...
# v1.3.3: Null check on ORACLE_PRICE_ABS before bc division.
#         Empty Chainlink response warns and skips — no bc error.
# ============================================================
echo ""
...
if [[ -z "$BAL0_PRIOR_RAW" || "$BAL0_PRIOR_RAW" -eq 0 || \
    ...
    # v1.3.3: Null check before bc — empty oracle response skips display
    ...
    if [ -n "$ORACLE_RAW" ]; then
        ORACLE_PRICE_ABS=$(echo "$ORACLE_RAW" | awk -F',' '{print $2}' \
            | tr -d ' ' | tr -d '-' | sed 's/\[.*\]//')
        if [[ -n "$ORACLE_PRICE_ABS" && "$ORACLE_PRICE_ABS" -gt 0 ]]; then
            ORACLE_ETH_USD=$(echo "scale=2; $ORACLE_PRICE_ABS / 100000000" | bc)
            echo "   Chainlink ETH/USD reference: \$$ORACLE_ETH_USD"
        else
            echo "⚠️  ORACLE WARNING: Empty or invalid Chainlink response — skipping display"
            WARNINGS=$((WARNINGS + 1))
        fi
    else
        echo "⚠️  ORACLE WARNING: Chainlink unavailable — skipping reference display"
        WARNINGS=$((WARNINGS + 1))
    fi
