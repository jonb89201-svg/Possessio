...
/**
 * @notice Mock SSV Network registry for Gauntlet tests.
 */
contract MockSSVNetwork_G {
    struct Cluster {
...
contract PossessioPaymentsGauntlet is Test {
    ...
    function setUp() public {
        ...
        ssvNetwork   = new MockSSVNetwork_G();
    ...
    function testFuzz_SweepDustBounded(uint256 sweepAmount) public {
        ...
        for (uint256 i = 0; i < sweepCount; i++) {
            ...
            // Defensive 2-day warp to clear cooldown unambiguously across fuzz iterations
    ...
    function test_Attack_ThresholdEventNoSpam() public {
        ...
        usdc.mint(address(payments), 1000 * 1e6);
        // 100% cbETH path — set output above threshold
        // 35 cbETH * 1.05 = 36.75 ETH (> 32 threshold)
        router.setCbEthOut(35 ether);

        vm.prank(MERCHANT);
        payments.sweep(0, 35 ether);
    ...
    function test_Sequence_MultiActionStateIntegrity() public {
        ...
        assertEq(payments.getCitadelProgress(), expectedGauge, "Gauge reflects cbETH holdings");
    }
