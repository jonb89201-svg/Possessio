 ...
 *             - Remainder  → staking (40% cbETH / 60% rETH)
 *
...
// IwstETH interface: reserved for post-launch Lido integration
...
contract PLATE is ERC20, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    // WSTETH_PCT removed — planned post-launch Lido integration
    ...
    function _swapETHToDAI(uint256 ethAmt) internal {
        ...
        // NOTE: minDAI = 0 means oracle failed — swap aborts cleanly
        // This prevents sandwich attacks under degraded oracle conditions
        uint256 minDAI = 0; // default — requires valid oracle to proceed
        ...
        // Oracle failure guard: abort DAI swap if no valid price
        // ETH routes to Treasury fallback instead of swapping unprotected
        if (minDAI == 0) {
            (bool ok,) = TREASURY.call{value: ethAmt}("");
            require(ok, "PLATE: Oracle failure -- DAI ETH fallback failed");
            return;
        }
    ...
    function _deployToStaking(uint256 ethAmt) internal {
        ...
        // wstETH: planned post-launch Lido integration
        emit StakingDeployed(cbAmt, 0, rAmt, block.timestamp);
    }
    ...
    function harvestYield() external nonReentrant notPaused {
        ...
        // wstETH yield: planned post-launch Lido integration
    ...
    function _getSpotPrice() internal view returns (uint256 priceX96) {
            ...
            returns (int56[] memory tickCumulatives, uint160[] memory)
        {
            ...
            int56 tickDelta = tickCumulatives[1] - tickCumulatives[0];
            int56 rawTick   = tickDelta / int56(uint56(10));
            // Check bounds before casting to int24 to prevent overflow
            if (rawTick > 887272 || rawTick < -887272) return 0;
            int24 spotTick  = int24(rawTick);
