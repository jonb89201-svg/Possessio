...
#
# Sub-check B: 10-block liquidity delta via liquidity() (veto)
#   liquidity() retained for delta math and Gate 4 tick concentration.
# ============================================================
echo ""
...
# --- Sub-check B: 10-Block Liquidity Delta ---
...
# cast call with type specifier returns decoded decimal — cast to-dec not needed
LIQ_NOW="$LIQ_NOW_RAW"
LIQ_PRIOR="$LIQ_PRIOR_RAW"

# NOTE: 15% delta threshold is calibrated for mature pools.
# On a thin bootstrapping pool (~0.1 ETH depth), a single LP position
# entering or exiting the active tick can exceed 15% without JIT activity.
# This will produce false vetoes during early pool life — expected behavior.
# Revisit and tighten threshold once pool has meaningful depth.
# Track as known operational condition, not a bug.
if [ "$LIQ_PRIOR" -eq 0 ]; then
    ...
    if [ "$SPIKE" -eq 1 ]; then
        echo "❌ JIT VETO: In-range liquidity shifted ${LIQ_DELTA}% in 10 blocks (threshold: 15%)"
