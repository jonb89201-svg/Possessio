...
- [x] **x402Core — mainnet** → DONE. Live at `0x60d867Af…6c05` as constellation organ (2026-07-28, `deploy/DEPLOYED_BASE_8453.md`), pool source[1], heart wiring in bytecode. Architect-confirmed connected to the live pool (2026-08-11).
- [ ] **V3 — mainnet deploy** → fork-proven (CREATE3 3/3), ready; deploy pending.
- [x] **PITI — gate OPEN, engine LIVE.** The x402 gate is satisfied: the deployed x402Core (`0x60d867Af…6c05`) **is the new PITI.sol** — its pay-per-call engine serves PITI directly (Architect ruling, 2026-08-11). No separate PITI contract; PITI's remaining build is the worker-layer data product (the index + toll routes) on top of the live engine.
...
- [x] **Factory → Pool wiring upgrade** → DONE, superseded before deploy. The live `PossessioFactory` carries the A-edit: `forceApprove(feeSink, DEPLOYMENT_FEE)` + `IPossessioPool(feeSink).receiveInfraFunds(...)` (`src/PossessioFactory.sol:247-248`) with the `isInfraSink()` brick-guard in the constructor. `feeSink()` reads the Heart on chain (`deploy/DEPLOYED_BASE_8453.md` §PossessioFactory); factory is pool source[0]. **Deploy fees are accounted, drawable pool revenue today** — the self-funding door is open. (Stale text previously here described the pre-A-edit design; verified stale 2026-08-11, Fathom.)
- [x] **x402Core → Pool wiring upgrade** (P-5) → DONE, fixed before the constellation flew. Current `src/PossessioX402Core.sol` carries `heartSink` (immutable), routes deployment-fee/settle surplus over cap one-way into the Heart via the accounted `receiveInfraFunds` door (option A), with the `isInfraSink()` brick-guard — see gauntlet tests `test_deploymentFee_overCap_routesToHeart` / `test_settleCall_surplusOverCap_routesToHeart`. Deployed at the reserved CREATE3 address as pool source[1]. (P-5 text preserved in `AUDIT_POOL_20260715.md`; verified fixed 2026-08-11, Fathom.)
...
- 🥃 **Whisky RWA — "The Bond"** (allocated tokenized bottles, bonded warehouse). STARTED. Spec `specs/SPEC_PossessioWhiskyMarket.md` (V1/V5 resolved, English live-event auction ratified). `src/PossessioWhiskyMarket.sol` v0.1 built — primary English auction module, **compiles vs real base/base-std B20 interfaces**. NON-PROVEN. **Requirement for the pool deploy** (§6: mine market CreateX salt → address → pool's immutable source set). Next: secondary order book, DoD suite via `base-foundryup` (§8), B20 issuance wrap + relay + app. Deploy gated on §7 legal (holding co, warehouse agreement, securities framing, jurisdiction).
  - 🚨 **ZERO TEST COVERAGE — verified 2026-07-16.** `src/PossessioWhiskyMarket.sol` (313 lines: `listAuction` / `bid` / `settle`, USDC escrow, bps fee skim, allowlist membership, below-reserve fail-safe, `nonReentrant`) has **NOT ONE test** — not adversarial, not happy-path. The only `whisky` reference in `test/` is a mock `makeAddr("whiskyMarket")` EOA in `PossessioPoolFork.t.sol`, which tests **the pool**, not this contract. It is the one contract in the repo that is completely dark. **HARD RULE: the whisky contract MUST NOT be deployed to a live pool-source address until it has its own gauntlet + DoD suite + the diverse council audit** (same bar as every launched contract). Authorizing its *address* in the pool source set is harmless (no code lives there until it deploys, exactly like the x402Core / factory authorize-at-construction pattern) — the gate is on the **contract deployment + live wiring**, not the address slot. See RUNBOOK §0.1.
...
>
> Underneath is a full, verifiable protocol. Live and code-verified on Base mainnet today: a payment engine (PossessioPayments) and a fail-closed cbETH valuation guard (LSTExchangeRate). Fork-tested and ready to deploy: a Uniswap V4 treasury engine with Chainlink automation, and an Ethereum settlement layer. Adversarial, invariant, and fork-proven throughout — run `forge test` for the live count. Check the contracts on-chain; run the tests yourself.
>
...
**Transactions page — UNIFIED, staged (decided 2026-07-22):** header **Transactions** button opens `#tx-view`, a 3-tab overlay.
- [x] **Stage 1 — Console tab (DONE):** records every write this console sends (hooked in `handleSend` → `TxHistory`, localStorage, device-local, 200 cap), with status pill (pending/success/reverted) and explorer link per chain (Base/Sepolia/Eth/Solana).
- [ ] **Stage 2 — Wallet tab:** connected wallet's on-chain history vs POSSESSIO contracts. Needs a worker proxy to a BaseScan/indexer read keyed to deployed addresses.
- [ ] **Stage 3 — Desk tab:** Rail enter/exit, FundingVault fund/draw/return, AutoTarget intents — reads their events; activates on trading-contract deploy + keeper.
- [ ] **Follow-on:** also record the testnet drip (`/api/testnet/drip`), payment, and deploy tx paths into `TxHistory` (only the main `handleSend` write path is hooked so far).
