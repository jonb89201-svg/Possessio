...
contract PLATELaunchTest is Test {
    ...
    function test_Launch_FirstLegitimateFeeSwapAfterBootstrap() public {
        ...
        // INSTRUMENTATION -- find exact source of residual PLATE
        console.log("=== PLATE ACCOUNTING TRACE ===");
        console.log("Before swapFeesToETH - plate balance:", plate.balanceOf(address(plate)));
        console.log("Before swapFeesToETH - pendingFees:", plate.pendingFees());

        // First fee swap must succeed
        plate.swapFeesToETH();

        console.log("After swapFeesToETH - plate balance:", plate.balanceOf(address(plate)));
        console.log("After swapFeesToETH - pendingFees:", plate.pendingFees());
        ...
        console.log("Before routeETH - plate balance:", plate.balanceOf(address(plate)));
        console.log("Before routeETH - pendingFees:", plate.pendingFees());
        console.log("Before routeETH - ETH balance:", address(plate).balance);

        plate.routeETH();

        console.log("After routeETH - plate balance:", plate.balanceOf(address(plate)));
        console.log("After routeETH - pendingFees:", plate.pendingFees());
        console.log("After routeETH - ETH balance:", address(plate).balance);
        console.log("minSwapBatch:", plate.minSwapBatch());
        ...
        // Strict assertion -- kept intentionally to expose accounting truth
        // This test SHOULD fail until source of residual PLATE is confirmed
        assertEq(plate.balanceOf(address(plate)), 0,
            "No PLATE stranded after full flywheel cycle");
    }
