...
// BUILD-PROOF: H1SLIP_V243_OUT61 — repointed to numberless PossessioPayments.sol.
...
contract MockMorpho_H1 {
    ...
    function deposit(uint256 a, address) external returns (uint256) {
        usdc.transferFrom(msg.sender, address(this), a); return a;
    }
...
contract PossessioPaymentsH1SlippageTest is Test {
    ...
    function test_H1_ZeroCallerMin_SandbaggedWeth_Reverts() public {
        ...
        vm.prank(MERCHANT);
        vm.expectRevert(); // oracle floor -> router slippage require fires
    ...
    function test_H1_CallerMinTighterThanFloor_IsHonored() public {
        ...
        vm.warp(block.timestamp + 7 days);
        vm.prank(MERCHANT);
        vm.expectRevert(); // 0.16 < caller's 0.20 -> router slippage require fires
    ...
    function test_H1_FloorScalesWithSweptAmount() public {
        ...
        vm.warp(block.timestamp + 7 days);
        vm.prank(MERCHANT);
        vm.expectRevert(); // scaled floor catches the proportionally-large sandbag
        payments.sweep(0, 0, 0);
    }
}
