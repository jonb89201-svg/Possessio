...
contract POSSESSIOv2Gauntlet is Test {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Proof Scope:    routeETH and harvestYield are guarded by nonReentrant.
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Proof Scope:    harvestYield NEVER withdraws below tracked principal.
    //                 No call sequence can drain staking principal via yield
    //                 harvest, even when depeg, dust, or repeated calls occur.
    // Boundary:       Balance == principal → revert ZeroAmount (no yield to take).
    //                 Balance < principal → impossible state, must revert.
    // Assumption Log: cbETH/rETH balances track principal correctly.
    ...
    function test_Attack_PrincipalErosionViaRepeatedHarvest() public {
        ...
        // Add small yield
        cbETH.addYield(address(hook), 0.01 ether);
        vm.deal(address(cbETH), address(cbETH).balance + 0.01 ether);
        rETH.addYield(address(hook), 0.01 ether);
        vm.deal(address(rETH), address(rETH).balance + 0.01 ether);
        ...
        uint256 rPrincipalAfter  = hook.rETHPrincipal();
        assertGt(cbPrincipalAfter, 0, "cbETH principal must remain");
        assertGt(rPrincipalAfter, 0, "rETH principal must remain");

        // Second harvest with no new yield must revert
        vm.expectRevert(PossessioHook.ZeroAmount.selector);
        hook.harvestYield();
    }

    function test_Attack_HarvestBeforeAnyStaking() public {
        // No deposits made — harvestYield must revert (nothing to harvest)
        vm.expectRevert(PossessioHook.ZeroAmount.selector);
        hook.harvestYield();
    }
    ...
    function test_Attack_HarvestDoesNotBypassPrincipal() public {
        ...
        uint256 principalBefore = hook.cbETHPrincipal() + hook.rETHPrincipal();

        // Set LST balance equal to principal (no yield)
        // harvest must revert
        vm.expectRevert(PossessioHook.ZeroAmount.selector);
        hook.harvestYield();

        uint256 principalAfter = hook.cbETHPrincipal() + hook.rETHPrincipal();
        assertEq(principalAfter, principalBefore, "Principal must not decrease on no-yield harvest");
    }
    ...
    function test_Attack_ForceInjectedETH_CannotMintStakingPrincipal() public {
        ...
        assertEq(hook.rETHPrincipal(),  rPrincipalBefore,  "rETH principal untouched");
    }
    ...
    // Boundary:       STEEL/DAI/cbETH/rETH/WETH always revert RescueBlocked.
    ...
    function test_Attack_RescueUnlistedTokenSucceeds() public {
        ...
        assertEq(randomToken.balanceOf(TREASURY), 100 * 1e18,
            "Non-protocol token must transfer to Treasury");
    }

    // ═══════════════════════════════════════════════════════════════════════
    //                  YIELD ATTACKS — NO AMPLIFICATION / DRIFT
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Proof Scope:    Repeated harvest cycles do not cause value drift,
    //                 recursive amplification, or cross-cycle state corruption.
    //                 Only ACTUAL yield is routed.
    // Boundary:       Multiple harvests at various yield amounts. Principal
    //                 tracked correctly across all cycles.
    // Assumption Log: Integer division truncates deterministically.

    function test_Attack_Yield_NoCrossCycleDrift() public {
        _fundAccumulator(1 ether);
        vm.prank(TREASURY);
        hook.routeETH();

        uint256 cbPrincipal0 = hook.cbETHPrincipal();
        uint256 rPrincipal0  = hook.rETHPrincipal();

        // Cycle 1: add yield, harvest
        cbETH.addYield(address(hook), 0.05 ether);
        vm.deal(address(cbETH), address(cbETH).balance + 0.05 ether);
        rETH.addYield(address(hook), 0.05 ether);
        vm.deal(address(rETH), address(rETH).balance + 0.05 ether);
        hook.harvestYield();

        // Principal unchanged
        assertEq(hook.cbETHPrincipal(), cbPrincipal0, "Cycle 1: cbETH principal drift");
        assertEq(hook.rETHPrincipal(),  rPrincipal0,  "Cycle 1: rETH principal drift");

        // Cycle 2: add yield, harvest
        cbETH.addYield(address(hook), 0.05 ether);
        vm.deal(address(cbETH), address(cbETH).balance + 0.05 ether);
        rETH.addYield(address(hook), 0.05 ether);
        vm.deal(address(rETH), address(rETH).balance + 0.05 ether);
        ...
        assertEq(hook.rETHPrincipal(),  rPrincipal0,  "Cycle 2: rETH principal drift");
    }

    function test_Attack_Yield_NoRecursiveAmplification() public {
        _fundAccumulator(1 ether);
        vm.prank(TREASURY);
        hook.routeETH();
        ...
        // Immediate second harvest — no new yield, must revert
        vm.expectRevert(PossessioHook.ZeroAmount.selector);
        hook.harvestYield();

        // Gain from first harvest is bounded (cannot amplify)
        assertLe(treasuryGain1, 0.1 ether, "Treasury gain bounded by yield amount");
    }
    ...
    function test_Attack_SAVBurnByNonCouncilMember() public {
        // Deposit some SAV first
        steel.approve(address(hook), 4000 * 1e18);
        vm.prank(TREASURY);
        // Need Treasury to hold STEEL for savDeposit
        ...
        // Attacker tries to burn from SAV
        vm.expectRevert(PossessioHook.OnlyCouncil.selector);
        vm.prank(ATTACKER);
    ...
    function test_Attack_SAVInventByNonCouncilMember() public {
        vm.expectRevert(PossessioHook.OnlyCouncil.selector);
        vm.prank(ATTACKER);
        hook.proposeInvent(100 * 1e18, "malicious");
    }
    ...
    function test_Attack_SAVSlashedBlocksAllOps() public {
        ...
        // All SAV ops must now revert
        vm.expectRevert(PossessioHook.SAVSlashedError.selector);
        vm.prank(TREASURY);
