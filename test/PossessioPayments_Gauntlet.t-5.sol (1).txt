...
// BUILD-PROOF: GAUNTLET_V243_OUT61b — H-1 slippage test alignment (consistent hop ratios), L-1 selectors, M-1 prank fix.
...
contract MockV3Router_G {
    ...
    MockWETH_G  public weth;   // v2.4.2 — USDC->WETH first hop of cbETH two-hop

    uint256 public daiOut;
    uint256 public wethOut;    // v2.4.2 — WETH delivered on USDC->WETH hop
    bool    public daiSwapReverts;
    bool    public wethSwapReverts;
    bool    public consumeLessThanRequested;

    // v2.4.2 — cbETH is no longer acquired via the V3 router. The contract's
    // cbETH leg routes USDC->WETH here, then WETH->cbETH through the Aerodrome
    // router. The old V3 cbETH branch/state was dead and has been removed; set
    // cbETH output on MockAerodromeRouter_G instead.
    constructor(address u, address d, address /*c (unused, kept for call-site arity)*/) {
        usdc = MockUSDC_G(u);
        dai  = MockDAI_G(d);
    }

    function setDaiOut(uint256 v) external { daiOut = v; }
    function setWethOut(uint256 v) external { wethOut = v; }       // v2.4.2
    function setWeth(address w) external { weth = MockWETH_G(w); } // v2.4.2
    function setDaiSwapReverts(bool b) external { daiSwapReverts = b; }
    function setWethSwapReverts(bool b) external { wethSwapReverts = b; } // v2.4.2
        ...
        external returns (uint256 amountOut)
    {
        ...
        // Mock V3 router stub for adversarial testing (dangling-approval scenario); production PossessioPayments uses SafeERC20 and approves a real V3 router on Base mainnet.
        ...
        if (p.tokenOut == address(dai)) {
            ...
            return daiOut;
        } else if (address(weth) != address(0) && p.tokenOut == address(weth)) {
            // v2.4.2 — USDC->WETH first hop. Preserves adversarial under-consume
            // semantics so leakage detection still triggers on this leg.
...
// v2.4.1 — MockChainlink_G with decimal-class support for constructor guard.
//          cbETH/ETH: 18-decimal exchange-rate (ORACLE_STALE_CBETH=90,000s)
//          DAI/ETH:   8-decimal price-feed (ORACLE_STALE=3,600s)
contract MockChainlink_G {
    ...
    function setStale() external {
        if (_decimals == 18) {
            _updatedAt = block.timestamp - 100_000;  // > ORACLE_STALE_CBETH
        } else {
            _updatedAt = block.timestamp - 7_200;    // > ORACLE_STALE
        }
    }
...
// v2.4.1 — MockWETH_G for multi-hop USDC→WETH→cbETH path
contract MockWETH_G {
...
// v2.4.1 — Adversarial Aerodrome router mock for WETH→cbETH leg.
//          Includes leakageMode, swapReverts, and consumeLessThanRequested
//          for full adversarial coverage parity with MockV3Router_G.
contract MockAerodromeRouter_G {
    MockWETH_G  public weth;
    ...
    constructor(address weth_, address cbeth_) {
        weth  = MockWETH_G(weth_);
        cbeth = MockCbETH_G(cbeth_);
    }
    ...
    function exactInputSingle(ExactInputSingleParams calldata p)
        external returns (uint256 amountOut)
    {
        require(!swapReverts, "MockAerodromeRouter_G: swap reverts");
        require(cbEthOut >= p.amountOutMinimum, "MockAerodromeRouter_G: slippage");
        callCount++;

        // Adversarial mode: consume less than requested (dangling-approval / leakage test)
        uint256 toConsume = consumeLessThanRequested ? p.amountIn / 2 : p.amountIn;
        if (leakageMode) toConsume = p.amountIn - 1;

        // forge-lint: disable-next-line(erc20-unchecked-transfer)
        MockWETH_G(p.tokenIn).transferFrom(msg.sender, address(this), toConsume);
...
// v2.4.1 — Adversarial Morpho vault mock for the 50% Morpho leg.
//          Supports depositReverts, leakageMode for dual-acquisition attack surface.
contract MockMorphoVault_G {
    MockUSDC_G public usdc;
    ...
    function deposit(uint256 assets, address receiver) external returns (uint256 shares) {
        require(!depositReverts, "MockMorphoVault_G: deposit reverts");
...
contract PossessioPaymentsGauntlet is Test {
    ...
    MockDAI_G              dai;
    MockCbETH_G            cbeth;
    MockWETH_G             weth;          // v2.4.1
    MockV3Router_G         router;
    MockAerodromeRouter_G  aeroRouter;    // v2.4.1
    MockMorphoVault_G      morphoVault;   // v2.4.1
    MockChainlink_G        chainlinkEth;
    MockChainlink_G        chainlinkDai;
    MockChainlink_G        chainlinkUsdcUsd;   // v2.4.2 — USDC/USD 8-dec
    MockChainlink_G        chainlinkEthUsd;    // v2.4.2 — ETH/USD 8-dec
    ...
    function setUp() public {
        ...
        router.setWeth(address(weth));   // v2.4.2 — enable USDC->WETH first hop
        aeroRouter   = new MockAerodromeRouter_G(address(weth), address(cbeth));
        morphoVault  = new MockMorphoVault_G(address(usdc));

        // v2.4.2 — Oracle scale:
        //   cbETH/ETH: 18-decimal exchange-rate, 0.98e18 healthy peg
        //   DAI/ETH:   8-decimal price-feed
        //   USDC/USD:  8-decimal (NEW v2.4.2) ; ETH/USD: 8-decimal (NEW v2.4.2)
        chainlinkEth     = new MockChainlink_G(int256(980_000_000_000_000_000), 18);
        chainlinkDai     = new MockChainlink_G(int256(500_000_000_000), 8);
        chainlinkUsdcUsd = new MockChainlink_G(int256(100_000_000), 8);      // $1.00
        chainlinkEthUsd  = new MockChainlink_G(int256(300_000_000_000), 8);  // $3000
        ...
        aeroRouter.setCbEthOut(1 ether);
    }

    /// @notice v2.4.2 — Known-good DeployParams for adversarial deploy sites.
    function _gauntletParams() internal view returns (PossessioPayments.DeployParams memory) {
    ...
    function testFuzz_SweepDustBounded(uint256 sweepAmount) public {
        ...
        for (uint256 i = 0; i < sweepCount; i++) {
            ...
            // H-1: mock returns must be realistic relative to input or the
            // oracle-derived slippage floor (correctly) rejects them as a
            // sandwich. With daiCeiling=0 and cbEthBps=5000, the cbETH leg
            // receives sweepAmount/2 USDC; at $3000/ETH that is
            // (sweepAmount/2)*1e12/3000 wei of WETH. Return ~100% of expected
            // (above the 90% floor) so an HONEST swap clears it. Dust is still
            // measured from the contract's USDC integer-division split, not the
            // swap return, so the invariant remains valid.
            uint256 cbEthAlloc = sweepAmount / 2;                  // 6-dec USDC
            uint256 expectedWeth = (cbEthAlloc * 1e12) / 3000;     // 18-dec WETH at $3000
            router.setWethOut(expectedWeth);
            // WETH->cbETH at 0.98 rate; return ~equal so it clears its own floor.
            aeroRouter.setCbEthOut((expectedWeth * 98) / 100);

            vm.prank(MERCHANT);
            payments.sweep(0, 0, 1);
    ...
    function test_Attack_MaliciousTokenCannotDrain() public {
        ...
        // v2.4.2 — start from known-good params, override only the DAI field
    ...
    function test_Attack_SweepCooldownBypass() public {
        ...
        usdc.mint(address(payments), 1000 * 1e6);
        router.setWethOut(0.31 ether); aeroRouter.setCbEthOut(0.31 ether);
    ...
    function test_Attack_DAIOracleRevertsSkipsGracefully() public {
        ...
        usdc.mint(address(payments), 1000 * 1e6);
        // v2.4.2 — cbETH is delivered by the Aerodrome WETH->cbETH leg, not the
        // V3 router. Set the amount on the leg that actually mints cbETH.
    ...
    function test_Recovery_FailureThenSuccess() public {
        ...
        // First sweep: cbETH leg fails. v2.4.2 — cbETH comes from the Aerodrome
        // WETH->cbETH leg, so inject the failure there (the V3 lst path is dead).
        router.setWethOut(0.31 ether);
        aeroRouter.setSwapReverts(true);
        aeroRouter.setCbEthOut(0.31 ether);
        ...
        // Fix the condition
        aeroRouter.setSwapReverts(false);
    ...
    function test_Sequence_MultiActionStateIntegrity() public {
        ...
        router.setWethOut(0); aeroRouter.setCbEthOut(0);
