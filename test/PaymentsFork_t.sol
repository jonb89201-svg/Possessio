...
contract PaymentsForkTest is Test {
    ...
    //
    // _validateOracle() reverts on a stale cbETH/ETH feed, but it does NOT check
    // USDC/USD or ETH/USD - so staling those two does not stop the sweep before
    // the WETH leg. These tests stale ONLY the USDC/ETH price feeds (keeping the
    // cbETH exchange-rate feed fresh so _validateOracle passes and execution
    // reaches the WETH leg), then sweep with minWethOut == 0.
    //
    // EXPECTED AFTER FIX: the WETH leg reverts OracleInvalid() when the oracle
    // ETH-equivalent is 0, regardless of the caller's min. The proposed two-line
    // guard on the WETH leg:
    //     uint256 ethEquiv = _usdcToEth(cbEthAlloc);
    //     if (ethEquiv == 0) revert OracleInvalid();
    //     uint256 wethFloor = (ethEquiv * SWEEP_SLIPPAGE_BPS) / SWEEP_SLIPPAGE_DENOM;
    //
    // CURRENT BEHAVIOR (no fix): the sweep does NOT revert on the stale feed - the
    // floor silently degrades to 0 and the leg swaps unprotected. So against the
    // unpatched contract these _reverts tests FAIL (red), documenting the residual.
    // After the guard is added they PASS (green). This is the terminal proof the
    // residual is real and the fix closes it.
    // ─────────────────────────────────────────────────────────────────────
    ...
    // Stale USDC/USD feed + minWethOut == 0  => WETH leg must NOT fail open.
    // Currently it does (no revert); after the OracleInvalid guard it reverts.
    function test_fork_sweep_staleUsdcUsd_zeroMinWeth_mustRevert() public {
        if (!forked) return;
        _seedForSweep();
        _staleFeed(CHAINLINK_USDC);                      // stale USDC/USD only
        // cbETH/ETH feed (CHAINLINK_CBETH) left fresh so _validateOracle passes
        // and execution reaches the WETH leg.
        vm.prank(OWNER);
        // EXPECTED AFTER FIX: revert OracleInvalid (stale feed -> no fail-open).
        vm.expectRevert(PossessioPayments.OracleInvalid.selector);
        payments.sweep(0, 0, 0);
    }

    // Stale ETH/USD feed + minWethOut == 0  => WETH leg must NOT fail open.
    ...
    // CONTROL: with the SAME stale feed but a REAL caller-supplied minWethOut, the
    // caller min is the active floor and the leg is protected by it. This proves
    // the fail-open is SPECIFICALLY the minWethOut==0 + stale-feed combination, and
    // that a keeper passing a real min is not exposed. (We assert a revert here too,
    // because with a high min the swap cannot meet it - but the point is the FLOOR
    // is active, not absent. If the contract instead reverts SweepTooEarly or
    // BatchTooSmall this setup is wrong - so we expect a swap/slippage-class revert,
    // not a clean success.) We use a deliberately unattainable min so the protected
    // leg reverts on slippage rather than swapping unprotected.
    function test_fork_sweep_staleUsdcUsd_realMinWeth_floorActive() public {
        if (!forked) return;
        _seedForSweep();
        _staleFeed(CHAINLINK_USDC);
        vm.prank(OWNER);
        // an absurdly high minWethOut: if the floor logic uses callerMin, the V3
        // swap cannot satisfy it and reverts (protected). This documents that a
        // real caller min IS honored even when the oracle floor degraded to 0.
        vm.expectRevert();   // V3 router "Too little received" or equivalent slippage revert
