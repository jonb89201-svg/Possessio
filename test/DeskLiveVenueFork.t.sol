...
///         vault.
///
///         Scoped asset: SOL on Base (0x3119...cf82, 9 decimals), which has a
///         DIRECT USDC pool — so this exercises V2's direct route
///         (viaWeth=false, feeA=3000, feeB=0). The WETH-hop route that makes
///         WETH-paired coins reachable is covered by the V2 Rail's own suites.
///         `exit` takes NO route arguments: it replays the entry recorded at
///         `enter`, so the mirror is structural rather than keeper discipline.
///
...
contract DeskLiveVenueForkTest is Test {
    ...
    address internal constant WETH   = 0x4200000000000000000000000000000000000006; // the ONE permitted hop
    uint256 internal constant REMOTE_TIMEOUT = 86_400; // 24h, per the deploy script header
    ...
    function setUp() public {
        ...
        remoteAgent = makeAddr("venue-remote-agent"); // dormant here; V2 requires non-zero
        owner    = address(this);
    ...
    function test_fork_SOL_round_trips_through_live_router() public {
        ...
        vm.prank(keeper);
        rail.enter(id, SOL, SIZE, 1, false, 3000, 0);
        uint256 probe = IERC20(SOL).balanceOf(address(rail));
        vm.revertToState(snap);

        uint256 floorOut = (probe * 99) / 100; // 1% below the observed fill
        vm.prank(keeper);
        rail.enter(id, SOL, SIZE, floorOut, false, 3000, 0); // direct 0.3% tier, real floor
    ...
    function test_fork_slippage_floor_rejects_an_unreachable_minimum() public {
        ...
        rail.enter(id, SOL, SIZE, probe * 2, false, 3000, 0);
