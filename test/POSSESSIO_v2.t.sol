...
contract POSSESSIOv2Test is Test {
    ...
    function test_Deploy_Hook_RevertsOnWrongDaiOracleDecimals() public {
        ...
        PossessioHook.DeployParams memory p = _baseParams(council);
        p.chainlinkDaiUsd = address(wrongClass);
        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        new PossessioHook(p);
    }
    ...
    // ===========================================================================
    //   DAI CROSS-RATE MANIPULATION / BINDING TESTS
    // ===========================================================================
    // These prove the ETH/USD ÷ DAI/USD slippage floor BINDS and FAILS CLOSED.
    // With no spot/TWAP symmetry guard possible in V4 (no pool oracle), this floor
    // is the ONLY manipulation defense shipping — so "it exists" is not enough; it
    // must (1) reject a swap that returns less DAI than the oracle-derived floor,
    // and (2) refuse to swap at all when either feed is unhealthy (never swap
    // unprotected). SCOPE NOTE: these prove the floor's LOGIC is correct and
    // fail-closed. They do NOT prove the 90% band is tight enough to stop a real
    // sandwich on a thin pool — that is a fork test against real liquidity, separate.

    // (1) FLOOR BINDS: healthy feeds, but the swap returns far less DAI than the
    //     cross-rate floor demands → must revert (manipulated/sandwiched outcome).
    function test_DAICrossRate_FloorRejectsInsufficientDAI() public {
        clCbETH.setAnswer(int256(980_000_000_000_000_000)); // healthy cbETH
        clDAI.setAnswer(int256(100_000_000));               // DAI/USD $1.00
        clEthUsd.setAnswer(int256(166_939_220_000));        // ETH/USD $1669.39

        _fundAccumulator(1 ether);
        // 1 wei DAI is far below any floor derived from ~1670 DAI/ETH at 90%.
        v3Router.setDAIReturn(1);

        vm.prank(TREASURY);
        vm.expectRevert(); // router's slippage revert — floor binds
        hook.routeETH();
    }

    // (2a) FAIL CLOSED — ETH/USD stale: warp past the 90,000s window, refresh only
    //      cbETH and DAI/USD, leave ETH/USD stale. The cross-rate can't compute →
    //      minDai stays 0 → valid false → DAI leg must NOT fire (no unprotected swap).
    function test_DAICrossRate_FailsClosed_WhenEthUsdStale() public {
        _fundAccumulator(1 ether);
        v3Router.setDAIReturn(100 * 1e18); // would-be-healthy payout

        vm.warp(block.timestamp + 90_001); // stale ALL feeds
        clCbETH.setAnswer(int256(980_000_000_000_000_000)); // refresh cbETH
        clDAI.setAnswer(int256(100_000_000));               // refresh DAI/USD
        // ETH/USD deliberately NOT refreshed → remains stale.
        ...
        assertEq(
            hook.daiReserve(),
            reserveBefore,
            "fail-closed: stale ETH/USD must prevent the DAI swap (no unprotected swap)"
        );
    }

    // (2b) FAIL CLOSED — DAI/USD feed reverts: the cross-rate can't read it →
    //      minDai stays 0 → DAI leg must NOT fire.
    function test_DAICrossRate_FailsClosed_WhenDaiUsdReverts() public {
        clCbETH.setAnswer(int256(980_000_000_000_000_000));
        clEthUsd.setAnswer(int256(166_939_220_000));
        clDAI.setReverts(true); // DAI/USD feed reverts on read

        _fundAccumulator(1 ether);
        v3Router.setDAIReturn(100 * 1e18);
        ...
        assertEq(
            hook.daiReserve(),
            reserveBefore,
            "fail-closed: reverting DAI/USD feed must prevent the DAI swap"
        );
    }
