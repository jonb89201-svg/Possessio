...
contract POSSESSIOv2Test is Test {
    ...
    // ===========================================================================
    //
    // v2.6.1 added external dispatchers (_routeETHDispatch, _harvestRewardsDispatch)
    // gated by msg.sender == address(this). Wrapper pattern: arm secret, try
    // dispatch, on revert clear secret and re-bubble via assembly revert.
    // Mitigates Grok finding: revert during execution could leave _activeExecutionSecret
    // armed for nonce-burn griefing attack across blocks.

    function test_XLink_DispatchWrapperRejectsExternalCaller() public {
        // v2.6.3 — _routeETHDispatch(secret, originalCaller, minDai, minCbETH).
        // External but gated by msg.sender == address(this). External caller
        // (including TREASURY) must be rejected with InvalidAddress before any
        // routing logic. Args are arbitrary; the gate fires first.
        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        hook._routeETHDispatch(1, USER, 0, 0);
    ...
    function test_XLink_HarvestDispatchRejectsExternalCaller() public {
        // v2.6.3 — _harvestRewardsDispatch(secret, originalCaller, minWETH).
        vm.expectRevert(PossessioHook.InvalidAddress.selector);
        hook._harvestRewardsDispatch(1, USER, 0);
    ...
    function testFuzz_ETHConservation(uint256 amt) public {
        ...
        uint256 treasuryBefore     = TREASURY.balance;
        uint256 wethRouterBefore   = weth.balanceOf(address(aeroRouter));
        ...
        uint256 wethRouterAfter    = weth.balanceOf(address(aeroRouter));
        uint256 hookETHAfter       = address(hook).balance;
        uint256 accumulatedAfter   = hook.accumulatedETH();

        // Sinks where ETH (or ETH-equivalent value) ended up:
        // - Treasury direct (75% portion minus DAI)
        // - WETH consumed by aeroRouter (representing staking allocation that became cbETH)
        // - Hook balance retained (any LP-portion stuck due to mock pool)
        // - accumulatedETH retained (any failed transient routing that needs retry)
        uint256 routedSum = (treasuryAfter - treasuryBefore)
                          + (wethRouterAfter - wethRouterBefore)
                          + hookETHAfter
                          + (accumulatedAfter > accumulatedBefore
                                ? (accumulatedAfter - accumulatedBefore)
                                : 0);

        // Conservation invariant holds within dust (rounding from integer division)
        assertApproxEqAbs(routedSum, amt - hookETHBefore + hookETHBefore, 1000 wei,
