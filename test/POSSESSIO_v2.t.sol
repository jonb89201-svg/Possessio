...
// BUILD-PROOF: BEFORESWAP_SLOT0_PIECE1_OUT63 + PIECE2_OUT64 (extreme price M-1 integration + take + D-1 delta) — extsload-backed MockPoolManager + Piece 1 slot0 plumbing test (feeETH==0 path).
import "forge-std/console.sol";
import "forge-std/StdStorage.sol";
import "../src/POSSESSIO_v2-6-3.sol";
// Piece 2 reads the returned BeforeSwapDelta's two halves. The hook imports only
// {BeforeSwapDelta, toBeforeSwapDelta}, not the accessor library, so bring it in
// here and read the delta the SAME way the contract's world packs it — no
// hand-rolled int256 bit-unpacking.
...
contract MockPoolManager {
    // ─── slot0 service for StateLibrary.getSlot0 (beforeSwap price read) ───
    // getSlot0 derives stateSlot = keccak256(abi.encodePacked(poolId, POOLS_SLOT))
    // with POOLS_SLOT = 6 (verified against v4-core StateLibrary) and reads
    // extsload(stateSlot). sqrtPriceX96 occupies the bottom 160 bits.
    ...
    function setSlot0(PoolId poolId, uint160 sqrtPriceX96) external {
        ...
        _store[stateSlot] = bytes32(uint256(sqrtPriceX96)); // tick/fees = 0
    }
    ...
    // beforeSwap calls take(Currency(0), hook, feeETH) on the feeETH>0 path.
    // Send real ETH so the hook's balance/accounting reflects the capture.
    // (Must be funded with ETH by the test before a non-zero-fee swap.)
    function take(Currency, address to, uint256 amount) external {
        (bool ok, ) = payable(to).call{value: amount}("");
        require(ok, "mock take: ETH send failed");
    }
...
contract POSSESSIOv2Test is Test {
    ...
    function testFuzz_ETHConservation(uint256 amt) public {
        ...
        // Conservation identity: the ETH that LEFT the hook (it is the source —
        // _fundAccumulator raised the hook's ETH balance by `amt`, and _routeETH
        // wraps/sends/settles it out) must equal the ETH-equivalent that ARRIVED
        // at every sink. The hook term is therefore a (before − after) outflow;
        // the sinks are (after − before) inflows. (The earlier form added
        // hookEthAfter − hookEthBefore as if the hook were a sink, which
        // underflows on uint256 because the hook's ETH balance DECREASES.)
        uint256 ethLeftHook = hookEthBefore - hookEthAfter;   // source outflow

        uint256 arrivedAtSinks =
            ...
            + (hookWethAfter    - hookWethBefore)      // WETH still held by hook (mid-flight residual)
        ...
        // What left the hook as ETH must reappear as ETH/WETH across the sinks,
        // within integer-division dust. (WETH the hook wrapped then sent shows up
        // at the routers; WETH it wrapped but retained shows up in hookWethAfter.)
        assertApproxEqAbs(arrivedAtSinks, ethLeftHook, 1000 wei,
    ...
    // ===========================================================================
    //   SLOT0 / beforeSwap HARNESS — PIECE 1: plumbing proof (feeETH==0 path)
    // ===========================================================================
    // Proves a chosen sqrtPriceX96 flows through the REAL StateLibrary.getSlot0
    // read inside beforeSwap. Uses a STEEL-specified swap with an amount tiny
    // enough that feeETH = (ethEquivalent * 200)/10000 rounds to 0, so beforeSwap
    // hits the early `return` BEFORE take() — isolating the slot0 wiring from the
    // delta/settlement path. If this passes, the swamp (slot derivation + extsload)
    // works; Piece 2 then layers extreme-price (M-1 integration) + take/delta (D-1).
    function _ethSteelKey() internal view returns (PoolKey memory k) {
        // currency0 = native ETH (address 0), currency1 = STEEL. Ordering matches
        // the hook's "fee always in ETH = currency0" assumption.
        k = PoolKey({
            currency0:   Currency.wrap(address(0)),
            currency1:   Currency.wrap(address(steel)),
            fee:         3000,
            tickSpacing: 60,
    ...
    function test_BeforeSwap_Slot0PlumbingFeedsKnownPrice() public {
        PoolKey memory k = _ethSteelKey();
        PoolId pid = PoolIdLibrary.toId(k);

        // Feed a normal price (1:1) into the slot the hook will read.
        uint160 sqrtP = uint160(1) << 96;
        poolManager.setSlot0(pid, sqrtP);

        // STEEL-specified, exactIn (!zeroForOne && amountSpecified < 0) → the
        // branch that reads price and does the staged-mulDiv conversion. Amount
        // of 1 wei makes feeETH round to 0 → early return before take().
        IPoolManager.SwapParams memory sp = IPoolManager.SwapParams({
            zeroForOne:        false,
            amountSpecified:   -1,            // exactIn, STEEL specified
            sqrtPriceLimitX96: 0
        });

        // beforeSwap is onlyPoolManager → call as the manager.
        vm.prank(address(poolManager));
        (bytes4 sel, , ) = hook.beforeSwap(address(this), k, sp, "");

        // Returning the correct selector proves beforeSwap executed end-to-end:
        // getSlot0 read our slot, the STEEL branch ran, feeETH==0 early-returned.
        assertEq(sel, hook.beforeSwap.selector, "beforeSwap must complete via slot0 read");
    }

    // ===========================================================================
    //   SLOT0 / beforeSwap HARNESS — PIECE 2: extreme price + take/delta
    // ===========================================================================
    // Built on Piece 1's proven plumbing. Drives an EXTREME sqrtPriceX96 (2^129,
    // above the 2^128 overflow threshold) through the real beforeSwap with a
    // non-zero feeETH, proving three things at once:
    //   (1) M-1 AT INTEGRATION LEVEL: the staged FullMath.mulDiv survives 2^129 in
    //       the live beforeSwap path. The pre-fix bare square (sqrtP*sqrtP) would
    //       revert here on overflow — so a green proves the fix holds end-to-end,
    //       not just in the standalone math test.
    //   (2) TAKE PATH: feeETH>0 → POOL_MANAGER.take moves ETH to the hook →
    //       accumulatedETH increases by exactly feeETH.
    //   (3) D-1 (BeforeSwapDelta sign): for a STEEL-specified swap the returned
    //       delta must be 0 on the specified side and +feeETH on the unspecified
    //       side (Balanced Delta: cancels the take's debit, net zero, ETH in hook).
    using BeforeSwapDeltaLibrary for BeforeSwapDelta;

    function test_BeforeSwap_ExtremePrice_M1Integration_And_D1Delta() public {
        PoolKey memory k = _ethSteelKey();
        PoolId pid = PoolIdLibrary.toId(k);

        // Extreme price: 2^129 > 2^128. Bare square 2^129*2^129 = 2^258 overflows
        // uint256 — the exact M-1 trigger — but the staged mulDiv must survive.
        uint160 sqrtP = uint160(uint256(1) << 129);
        poolManager.setSlot0(pid, sqrtP);

        // STEEL-specified, exactIn, 1 wei. At 2^129 this yields ethEquivalent =
        // 1 * 2^162 / 2^96 = 2^66 (~74 ETH) → feeETH = 2% ≈ 1.48 ETH: large enough
        // to be non-zero (exercises take + delta) yet fundable and well under int128.
        IPoolManager.SwapParams memory sp = IPoolManager.SwapParams({
            zeroForOne:        false,
            amountSpecified:   -1,            // exactIn, STEEL specified (!isETHSpecified)
            sqrtPriceLimitX96: 0
        });

        // feeETH the contract will compute (mirrors beforeSwap's staged math):
        // ethEquivalent = 1 * (2^129)^2 / 2^192 = 2^66 ; feeETH = ethEquivalent*200/10000.
        uint256 ethEquivalent = (uint256(1) << 66);
        uint256 expectedFee   = (ethEquivalent * 200) / 10000;

        // Fund the mock manager so its take() can send feeETH to the hook.
        vm.deal(address(poolManager), expectedFee + 1 ether);

        uint256 accBefore = hook.accumulatedETH();

        vm.prank(address(poolManager));
        (bytes4 sel, BeforeSwapDelta delta, ) = hook.beforeSwap(address(this), k, sp, "");

        // (1) M-1 integration: no revert at the extreme price.
        assertEq(sel, hook.beforeSwap.selector, "extreme-price beforeSwap must not revert (M-1 integration)");

        // (2) take path: accumulatedETH rose by exactly feeETH.
        assertEq(hook.accumulatedETH() - accBefore, expectedFee,
            "accumulatedETH must increase by feeETH (take path)");

        // (3) D-1: STEEL-specified → 0 on specified side, +feeETH on unspecified side.
        int128 specified   = delta.getSpecifiedDelta();
        int128 unspecified = delta.getUnspecifiedDelta();
        assertEq(int256(specified), int256(0),
            "D-1: specified-side delta must be 0 for STEEL-specified swap");
        assertEq(int256(unspecified), int256(uint256(expectedFee)),
            "D-1: unspecified-side delta must be +feeETH (Balanced Delta sign)");
    }
