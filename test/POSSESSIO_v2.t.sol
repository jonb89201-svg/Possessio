// BUILD-PROOF: V2CONS_DRAIN_OUT59 — source-drain conservation, signed math, underflow-proof.
...
contract POSSESSIOv2Test is Test {
    ...
    function testFuzz_ETHConservation(uint256 amt) public {
        ...
        // The hook is the SOURCE here: _fundAccumulator seeds `amt` into both
        // accumulatedETH and the hook's ETH balance, so measuring the hook's
        // closing balance as a positive sink underflows (after < before on the
        // source). Measure "net ETH that LEFT the hook" against "net value that
        // ARRIVED at the external venues" — every subtraction below is oriented
        // so the minuend is always >= the subtrahend, so no uint256 underflow.
        ...
        // ETH that actually drained from the hook (source). The hook only ever
        // spends ETH in routeETH, so before >= after — safe subtraction. Any
        // leg that retains (e.g. LP add reverting against the MockPoolManager
        // stub) simply keeps its ETH in-hook, reducing the drain accordingly.
        uint256 hookEthDrained  = hookEthBefore - hookEthAfter;

        // Value that arrived at the external venues, in ETH-equivalent (1 WETH
        // ≡ 1 ETH at the wrap boundary). The swap mocks pull WETH IN from the
        // hook, so v3/aero balances only rise — non-negative. Net out any WETH
        // still parked in the hook at the end (a leg that wrapped ETH→WETH but
        // whose external send did not move it), since that value did not reach
        // a venue. Wrapped via signed math to stay underflow-proof regardless
        // of which legs succeed or retain.
        int256 arrivals =
              int256(v3WethAfter   - v3WethBefore)     // DAI leg: WETH consumed by V3 router
            + int256(aeroWethAfter - aeroWethBefore)   // staking leg: WETH consumed by Aerodrome
            - int256(hookWethAfter - hookWethBefore);  // less WETH still parked in the hook

        // Conservation: what drained from the source equals what reached the
        // venues, within integer-division dust across the three-way split.
        assertApproxEqAbs(int256(hookEthDrained), arrivals, 1000 wei,
            "ETH conservation: hook drain must equal venue arrivals within dust");
    }
