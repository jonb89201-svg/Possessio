...
// BUILD-PROOF: DAI_CROSSRATE_TESTS_OUT64 — setUp re-tuned to DAI/USD+ETH/USD mocks; 3 manipulation/fail-closed tests added.
...
contract POSSESSIOv2Test is Test {
    ...
    MockChainlink        clDAI;     // now DAI/USD (8-dec) — repurposed from DAI/ETH
    MockChainlink        clEthUsd;  // ETH/USD (8-dec) — new, for the cross-rate
    ...
    function setUp() public {
        ...
        clDAI       = new MockChainlink(int256(100_000_000), 8);      // DAI/USD = $1.00 (8-dec)
        clEthUsd    = new MockChainlink(int256(166_939_220_000), 8);  // ETH/USD = $1669.39 (8-dec)
    ...
    // v3 — test_Deploy_Hook_RevertsOnWrongDaiOracleDecimals removed: the DAI/USD
    // feed (and its decimal-class guard) was cut with the DAI leg. The cbETH/ETH
    // decimal guard is still covered by test_Deploy_Hook_RevertsOnWrongCbEthOracleDecimals.
    ...
    function test_Deploy_Hook_ConstantsMatchV1() public {
        ...
        // v3 — DAI_BOOT_PCT assertion removed (constant deleted; no reserve carve-out).
    ...
    function test_Staking_CbETH100Percent() public {
        ...
        // v3 — Staking acquires cbETH via Aerodrome WETH→cbETH swap.
        // Track cbETH balance gain at hook (cbETHPrincipal increments in cbETH-wei).
        // 75% of 1 ETH = 0.75 ETH wraps to WETH and swaps to cbETH at 1:1 mock
        // ratio (full treasury portion — no reserve carve-out; Payments holds the reserve).
        uint256 cbBefore = cbETH.balanceOf(address(hook));

        // Configure router to return exactly the expected cbETH amount
        aeroRouter.setCbEthReturn(0.75 ether);
        ...
        // At 1:1 mock ratio, hook receives 0.75 cbETH for 0.75 WETH swapped
        assertApproxEqAbs(cbDelta, 0.75 ether, 1, "Hook must receive cbETH from staking allocation (75% of total)");
    }
    ...
    // v3 — test_Rescue_BlocksDAI removed (DAI no longer a protected immutable;
    // cbETH/WETH/STEEL rescue-block still covered below).
    ...
    function testFuzz_ETHConservation(uint256 amt) public {
        ...
        // Scale router return to match expected staking allocation (75% of amt — full treasury portion, SLIPPAGE-adjusted)
        ...
        // ── Conservation (v3, underflow-safe) ────────────────────────────
        // The hook is the SOURCE: _fundAccumulator deals it `amt` ETH, so its
        // own ETH balance DECREASES as it wraps→sends to staking. The prior
        // `hookEthAfter - hookEthBefore` term underflowed on that decrease.
        // Correct invariant: everything that left accumulatedETH must reappear
        // as ETH-equiv across external sinks PLUS whatever the hook retained.
        // Use signed deltas so a source can go negative without panicking.
        int256 treasuryDelta = int256(treasuryEthAfter) - int256(treasuryEthBefore);
        int256 poolDelta     = int256(poolMgrEthAfter)  - int256(poolMgrEthBefore);
        int256 hookEthDelta  = int256(hookEthAfter)     - int256(hookEthBefore);  // may be NEGATIVE (source spends)
        int256 hookWethDelta = int256(hookWethAfter)    - int256(hookWethBefore);
        int256 v3Delta       = int256(v3WethAfter)      - int256(v3WethBefore);
        int256 aeroDelta     = int256(aeroWethAfter)    - int256(aeroWethBefore);

        // Net ETH-equivalent that ended up SOMEWHERE = sum of all venue deltas.
        // The hook's own balance change is included with its true (signed) value:
        // ETH it released became WETH at aero/v3 (those deltas are positive),
        // ETH it retained (LP-failed) keeps hookEthDelta from fully cancelling.
        // Conservation: the total redistributed equals `amt` within dust, i.e.
        // the sum of where ETH *arrived* (positive deltas at sinks) offset by the
        // hook's net change reconstructs the routed amount.
        int256 routedSumSigned =
              treasuryDelta
            + poolDelta
            + hookEthDelta
            + hookWethDelta
        ...
        // All `amt` came from the hook's pre-dealt balance and accumulatedETH was
        // zeroed; the venue deltas net to ~0 because the hook's ETH decrease is
        // matched by equal WETH increases at the routers (wrap is value-neutral),
        // and the LP-retained ETH stays in the hook (hookEthDelta carries it).
        // Net redistribution across all venues must be ~0 (value neither created
        // nor destroyed), within integer-division dust.
        assertApproxEqAbs(routedSumSigned, int256(0), 1000 wei,
            "ETH conservation: value neither created nor destroyed across all venues");
    }
