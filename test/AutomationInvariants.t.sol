...
// BUILD-PROOF: AUTOINV_V243_OUT61b — Payments import numberless; Payments-SCH handshake tests REMOVED (contract gutted); hook X-LINK invariant retained (hook unchanged until green).
...
/*===============================================================================
  ...
  authored against verified POSSESSIO_v2-6-3.sol (hook, unchanged until full green) and PossessioPayments.sol (numberless) source signatures.
  Migrated from v2-4/v2-2 by Plate COUNCIL_2 (V3 cbETH False Green resolution).
...
// V3 — MockCbETH locked down to match new IcbETH interface (cbETH False Green
//      resolution). Old deposit()/withdraw() removed; only balanceOf + approve
//      consumed by contract. mint/addRewards preserved as test helpers.
contract MockCbETH {
...
// V3 — MockChainlink updated for asymmetric staleness windows and decimal-class
//      guard. cbETH/ETH=18-dec (ORACLE_STALE_CBETH=90,000s).
//      DAI/ETH=8-dec (ORACLE_STALE=3,600s).
contract MockChainlink {
    ...
    /// @notice Class-appropriate staleness window per V3.
...
// V3 — MockAerodromeRouter for WETH↔cbETH swap path (cbETH False Green
//      resolution). Replaces v2.4 cbETH.deposit/withdraw direct calls.
contract MockAerodromeRouter {
        ...
        external payable returns (uint256 amountOut)
    {
        ...
        if (params.tokenIn == weth && params.tokenOut == cbETH) {
            ...
            if (wethReturn > 0) {
                MockWETH(payable(weth)).transfer(params.recipient, wethReturn);
            }
            return wethReturn;
        }
...
contract AutomationInvariants is Test {
    ...
    MockAerodromeRouter aeroRouter;  // V3 — cbETH False Green resolution
    MockChainlink       clCbETH;
    MockChainlink       clDAI;     // DAI/USD (8-dec) — repurposed from DAI/ETH
    MockChainlink       clEthUsd;  // ETH/USD (8-dec) — new, for cross-rate
    ...
    function setUp() public virtual {
        ...
        // V3 — Oracle scale migrated: cbETH/ETH 18-decimal exchange-rate (was
        // 8-decimal in v2.4 — cbETH False Green). 0.98e18 = healthy peg.
        clCbETH     = new MockChainlink(int256(980_000_000_000_000_000), 18);
        clDAI       = new MockChainlink(int256(100_000_000), 8);      // DAI/USD $1.00
        clEthUsd    = new MockChainlink(int256(166_939_220_000), 8);  // ETH/USD $1669.39

        // STEEL token
        steel = new STEEL(address(this));

        // PossessioHook -- build DeployParams struct.
        // V3 — DeployParams adds aeroRouter field.
        ...
        PossessioHook.DeployParams memory p = PossessioHook.DeployParams({
            ...
            chainlinkDaiUsd: address(clDAI),
            chainlinkEthUsd: address(clEthUsd),
            v3Router:       address(v3Router),
            aeroRouter:     address(aeroRouter),  // V3
            weth:           address(weth),
            council:        council
        });
        ...
        // V3 — Pre-fund aeroRouter for swap returns
        aeroRouter.setCbEthReturn(1 ether);
        // Harvest direction (cbETH->WETH): set the WETH delivered per swap.
        // Without this, wethReturn defaults to 0, the mock's slippage require
        // (wethReturn >= minWETH) fails, and the contract's try/catch surfaces
        // ZeroAmount(). 0.002 ether covers the 0.001764 ether minWETH for the
        // HARVEST_DUST_FLOOR*2 reward seed at the 0.98 peg / 90% slippage.
        aeroRouter.setWethReturn(0.002 ether);
        ...
        // Invariant fuzzer scope: restrict to the hook. The AUTO-INV invariants
        // assert hook-state properties; leaving the mocks in the target set let
        // the fuzzer arm adversarial mock modes (setLeakageMode, setSwapRevert)
        // that poison the invariants' own preconditions at setup time.
...
contract AutomationInvariantsNoForwarder is Test {
    ...
    function setUpNoForwarder() public {
        ...
        MockAerodromeRouter aeroRouter  = new MockAerodromeRouter(address(weth), address(cbETH));

        // V3 — 18-decimal cbETH/ETH oracle (was 8-decimal in v2.4)
        MockChainlink   clCbETH     = new MockChainlink(int256(980_000_000_000_000_000), 18);
        MockChainlink   clDAI       = new MockChainlink(int256(100_000_000), 8);      // DAI/USD $1.00
        MockChainlink   clEthUsd    = new MockChainlink(int256(166_939_220_000), 8);  // ETH/USD $1669.39
...
// V3 — MockChainlink_A updated for decimal-class guard.
//      cbETH/ETH: 18-decimal exchange-rate, 0.98e18 healthy default.
...
contract MockChainlink_A {
    ...
    constructor(uint8 decimals_) {
        ...
        if (decimals_ == 18) {
            ...
            answer = int256(100_000_000);              // $1.00 in 8-dec (USD feed default)
        }
    }
    function latestRoundData() external view returns (
        uint80, int256, uint256, uint256, uint80
...
// V3 — WETH mock for PossessioPayments multi-hop USDC→WETH→cbETH path
contract MockWETH_A is MockUSDC_A {
    constructor() { decimals = 18; symbol = "WETH"; name = "Wrapped Ether"; }
}

// V3 — Aerodrome router stub for PaymentsAutomation substrate
...
// V3 — Morpho vault stub for PaymentsAutomation substrate
...
contract PaymentsAutomationInvariants is Test {
    ...
    MockAerodromeRouter_A   aeroRouter;    // V3
    MockMorphoVault_A       morphoVault;   // V3
    ...
    function test_PaymentsManualSweepWorksWithoutAutomation() public {
        ...
        vm.prank(OPERATOR);
        payments.sweep(0, 0, 0); // should succeed via OPERATOR_ROLE
    }
