...
contract PossessioRailAdversarialTest is Test {
    ...
    function _enter(uint256 id, uint256 size) internal { _open(id,1); router.setOut(100e18); vm.prank(keeper); rail.enter(id, address(token), size, 1e18, FEE); }

    // ─────────────────────── zero / input guards ───────────────────────────
    function test_enter_zeroToken() public { _open(1,1); vm.prank(keeper); vm.expectRevert(PossessioRail.ZeroAddress.selector); rail.enter(1, address(0), 1_000e6, 1e18, FEE); }
    function test_enter_zeroSize() public { _open(1,1); vm.prank(keeper); vm.expectRevert(PossessioRail.ZeroAmount.selector); rail.enter(1, address(token), 0, 1e18, FEE); }
    function test_enter_zeroMinOut_reverts() public { _open(1,1); vm.prank(keeper); vm.expectRevert(PossessioRail.ZeroMinOut.selector); rail.enter(1, address(token), 1_000e6, 0, FEE); }
    function test_exit_zeroMinOut_reverts() public { _enter(1, 1_000e6); at.setStatus(1,2); vm.prank(keeper); vm.expectRevert(PossessioRail.ZeroMinOut.selector); rail.exit(1, 0, FEE); }

    // ─────────────────────── role boundaries ───────────────────────────────
    function test_attacker_cannotEnter() public { _open(1,1); vm.prank(attacker); vm.expectRevert(PossessioRail.NotKeeper.selector); rail.enter(1, address(token), 1_000e6, 1e18, FEE); }
    function test_attacker_cannotExit() public { _enter(1,1_000e6); at.setStatus(1,2); vm.prank(attacker); vm.expectRevert(PossessioRail.NotKeeper.selector); rail.exit(1, 1, FEE); }
    function test_attacker_cannotOwnerExit() public { _enter(1,1_000e6); vm.prank(attacker); vm.expectRevert(PossessioRail.NotOwner.selector); rail.ownerExit(1, 1, FEE); }
    function test_keeper_cannotOwnerExit() public { _enter(1,1_000e6); vm.prank(keeper); vm.expectRevert(PossessioRail.NotOwner.selector); rail.ownerExit(1, 1, FEE); }
    ...
    function test_keeper_cannotSell_openIntent() public {
        ...
        vm.prank(keeper); vm.expectRevert(PossessioRail.ExitNotAuthorized.selector); rail.exit(1, 1, FEE);
    }
    ...
    function test_exit_unknownPosition_reverts() public {
        _open(9,2); vm.prank(keeper); vm.expectRevert(PossessioRail.PositionNotOpen.selector); rail.exit(9, 1, FEE);
    }
    ...
    function test_outstandingCap_blocksThirdConcurrent_noPosition() public {
        ...
        rail.enter(3, address(token), MAX_PER_TRADE, 1e18, FEE);
    ...
    function test_buy_slippage_enforced_atRouter() public {
        ...
        rail.enter(1, address(token), 1_000e6, 401e18, FEE);
    }
    ...
    function test_deadKeeper_ownerCanStillExit() public {
        ...
        vm.prank(owner); rail.ownerExit(1, 1, FEE);        // owner is never trapped
    ...
    function test_reenter_afterClose_reverts() public {
        _enter(1, 3_000e6); at.setStatus(1,2); router.setOut(3_600e6);
        vm.prank(keeper); rail.exit(1, 1, FEE);            // Closed
        _open(1,1);
        vm.prank(keeper);
    ...
    function test_ownerExit_thenExit_reverts() public {
        ...
        vm.prank(owner); rail.ownerExit(1, 1, FEE);        // owner closed it
        at.setStatus(1,2);
        vm.prank(keeper);
