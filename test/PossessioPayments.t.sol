...
import "../src/PossessioPayments_v2-2.sol";
...
/*═══════════════════════════════════════════════════════════════════════════════
  ...
  Surface covered:
    ...
    - dispatchSweep + OPERATOR_ROLE-to-self setup requirement
...
contract PossessioPaymentsAutomationTest is Test {
    ...
    function setUp() public {
        ...
        // Constructor signature: matches v2-1 (unchanged in v2-2).
        // Adjust args here if canonical test setUp differs — this is best-effort skeleton.
        payments = new PossessioPayments(
            OWNER,
            address(usdc),
            address(dai),
    ...
    function test_CheckUpkeep_FalseDuringSweepDelay() public {
        ...
        usdc.mint(address(payments), aboveHysteresis);
        // No skip — within SWEEP_DELAY of lastSweepTime=0 (but block.timestamp is fresh,
        // so this only matters after a successful sweep). Test the post-sweep delay.

        // First sweep manually
        vm.prank(OWNER);
        payments.sweep(0, 0);

        // Mint more USDC immediately, check upkeep BEFORE SWEEP_DELAY
        usdc.mint(address(payments), aboveHysteresis);
        (bool needed,) = payments.checkUpkeep("");
    ...
    function test_SetSweepSlippageGuards_OwnerOnly() public {
        vm.prank(ATTACKER);
        vm.expectRevert();
        payments.setSweepSlippageGuards(100, 200);
    }

    function test_SetSweepSlippageGuards_StoresValues() public {
        vm.prank(OWNER);
        payments.setSweepSlippageGuards(123, 456);
    ...
    function test_CheckUpkeep_PerformDataCarriesSlippageGuards() public {
        vm.prank(OWNER);
        payments.setSweepSlippageGuards(111, 222);
        ...
        (uint256 minDai, uint256 minCbEth) = abi.decode(data, (uint256, uint256));
        assertEq(minDai, 111);
    ...
    function test_DispatchSweep_RevertNonSelf() public {
        vm.prank(ATTACKER);
        vm.expectRevert(PossessioPayments.NotSelf.selector);
        payments.dispatchSweep(0, 0);
    }

    function test_PerformUpkeep_RequiresOperatorRoleGrantedToSelf() public {
        // Without granting OPERATOR_ROLE to address(payments) itself,
        // dispatchSweep → this.sweep() will revert because msg.sender (address(this))
        // has no role.
        vm.prank(OWNER);
        payments.setForwarder(FORWARDER);
        ...
        vm.prank(FORWARDER);
        vm.expectRevert();  // sweep() will revert due to missing role on address(this)
        payments.performUpkeep(abi.encode(uint256(0), uint256(0)));
    }

    function test_PerformUpkeep_SucceedsWithOperatorRoleGrantedToSelf() public {
        vm.prank(OWNER);
        payments.setForwarder(FORWARDER);

        // CRITICAL SETUP STEP — documented in PossessioPayments_v2-2.sol header:
        vm.prank(OWNER);
        ...
        // Now performUpkeep should succeed (or revert for legitimate reasons
        // like sweep mechanics, not for role reasons)
        vm.prank(FORWARDER);
        try payments.performUpkeep(abi.encode(uint256(0), uint256(0))) {
            // success path
            assertEq(payments.executedUpkeepCount(), 1);
        } catch (bytes memory reason) {
            // Acceptable if sweep itself reverts for unrelated reasons (oracle, etc.)
            // What we're verifying is NOT a role-related revert.
            emit log_named_bytes("performUpkeep revert (acceptable if not role-related)", reason);
        }
    }
