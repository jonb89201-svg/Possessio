...
contract MockV3Router {
        ...
        external payable returns (uint256 amountOut)
    {
        ...
        if (params.tokenIn == weth && params.amountIn > 0) {
            // forge-lint: disable-next-line erc20-unchecked-transfer — Mock V3 router stub for testing; production hook approves and uses SafeERC20-equivalent V3 router on Base mainnet.
...
contract POSSESSIOv2Test is Test {
    ...
    function test_RouteETH_AllocationsSum100Percent() public {
        ...
        //   80% of 75% = 60% → cbETH staking (100% allocation)
        uint256 cbAfter = address(cbETH).balance;

        // Staking deposit check (60% of total → 100% to cbETH)
        uint256 cbDelta = cbAfter - cbBefore;
        assertGt(cbDelta, 0, "Staking deposits must occur");
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Proof Scope:    _deployToStaking sends 100% to cbETH, increments principal
    //                 counter, forwards to Treasury on deposit failure (P2.2 fix)
    //                 or when cbETH is paused via depeg guard.
    // Boundary:       cbETHPaused redirects 100% to Treasury as raw ETH.
    //                 Deposit failure → Treasury fallback.
    // Assumption Log: cbETH contract accepts payable deposit() and returns
    //                 cbETH tokens 1:1 in this mock.
    // Non-Proven:     Does not prove real cbETH contract on Base mainnet
    //                 accepts this deposit flow (fork test required).
    ...
    function test_Staking_CbETH100Percent() public {
        ...
        // Expected: cbDelta = 0.60 ETH (100% of staking allocation)
        // Allow 1 wei rounding tolerance
        assertApproxEqAbs(cbDelta, 0.60 ether, 1, "cbETH must receive 100% of staking (60% of total)");
    }
    ...
    function test_Staking_CbETHPausedForwardsToTreasury() public {
        ...
        // On depeg: cbAmt = 0, full staking allocation forwarded to Treasury as raw ETH
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Proof Scope:    harvestRewards withdraws only above principal, splits 25/75
    //                 LP/Treasury, handles LP failure via Treasury fallback (H-2).
    // Boundary:       Balance == principal → revert (no rewards). Balance > principal
    //                 → withdraw delta. LP failure → fold into treasury portion.
    // Assumption Log: cbETH.withdraw returns native ETH 1:1.
    ...
    function test_Harvest_OnlyWithdrawsAbovePrincipal() public {
        // Seed: 1 ETH principal in cbETH, 0 rewards
        _fundAccumulator(1 ether);
        vm.prank(TREASURY);
        hook.routeETH(); // deposits principal

        // No rewards added — harvest reverts
        vm.expectRevert(PossessioHook.ZeroAmount.selector);
        hook.harvestRewards();
    }
    ...
    function test_Harvest_WithdrawsRewardsOnly() public {
        ...
        // Treasury gets full 0.1 ETH rewards (75% + 25% LP fallback).
        assertApproxEqAbs(treasuryGain, 0.1 ether, 1 wei, "Treasury must get 100% of rewards when LP fails in mock");
    }
    ...
    // Boundary:       STEEL/DAI/cbETH/WETH must revert RescueBlocked.
