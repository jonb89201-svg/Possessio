...
contract PLATEStakingTest is Test {
    ...
    function test_Stake_RevertsIfSAVNotSet() public {
        // SAV not locked yet — SAV_CONTRACT is address(0)
        // onlySAV fires first: msg.sender != address(0) → OnlySAV
        vm.prank(SAV_ADDR);
...
contract SAVTest is Test {
    ...
    function test_ProposeInvent_ReproposalAfterExpiryAllowsFreshApprovals() public {
        _deposit(DEPOSIT_AMOUNT);
        ...
        (uint8 approvalsBefore,,) = sav.getProposalStatus(HASH_A);
        assertEq(approvalsBefore, 3);
        // Expire the proposal
        vm.warp(block.timestamp + 31 days);
        // Re-propose — all prior approvals must be cleared
        vm.prank(C0); sav.proposeInvent(HASH_A);
        (uint8 approvalsAfter,,) = sav.getProposalStatus(HASH_A);
        assertEq(approvalsAfter, 0);
        // All four members can approve fresh
        vm.prank(C0); sav.approveInvent(HASH_A);
        vm.prank(C1); sav.approveInvent(HASH_A);
        vm.prank(C2); sav.approveInvent(HASH_A);
        vm.prank(C3); sav.approveInvent(HASH_A);
        (uint8 finalApprovals,,) = sav.getProposalStatus(HASH_A);
        assertEq(finalApprovals, 4);
    }
