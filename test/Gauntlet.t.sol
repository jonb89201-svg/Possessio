...
contract PLATEAttackTest is Test {
    ...
    function test_Harvest_DepegDoesNotBypassPrincipal() public {
        ...
        // Trigger depeg — oracle drops below 97% threshold
    ...
    // Proves: economic soundness of the complete harvest cycle
    // ============================================================
    ...
    // Oracle swings 94% → 98.5% → 95% during recovery window
    ...
    function test_Harvest_OracleJitter_CannotShavePrincipal() public {
        ...
        // Phase 1: depeg — oracle drops to 94%
        clCbETH.setAnswer(int256(94_000_000));
        vm.deal(address(plate), 1 ether);
        plate.routeETH();
        assertTrue(plate.cbETHPaused(), "Must be paused after depeg");

        // Phase 2: jagged recovery spike — oracle jumps to 98.5%
        clCbETH.setAnswer(int256(98_500_000));
        vm.deal(address(plate), 1 ether);
        plate.routeETH();

        // Phase 3: back below threshold — oracle drops to 95%
        clCbETH.setAnswer(int256(95_000_000));
        vm.deal(address(plate), 1 ether);
        plate.routeETH();

        // KEY ASSERTION: oracle cannot alter LST token balance
        // balanceOf returns token units — not market-priced ETH value
        ...
        // Principal completely intact after full jitter sequence
        assertEq(plate.cbETHPrincipal(), principalBefore,
