...
contract PLATEAttackTest is Test {
    ...
    // PLATE must not be stranded - recovered to Treasury
    // ETH stays in contract for downstream DAI/staking (V3 isolation)
    // ============================================================
    function test_Attack_V2LP_SwapSucceedsLPFails() public {
        router.setLiqRevert(true); // LP will fail
        router.setDAIReturn(1000 * 1e18);

        // Seed router with PLATE so swap can succeed
        ...
        // No PLATE stranded in contract (partial execution recovery)
        assertEq(plate.balanceOf(address(plate)), 0,
            "No PLATE may be stranded after LP failure");

        // No ETH stranded - downstream consumed it or staking received it
        assertLt(address(plate).balance, 0.01 ether,
            "No ETH stranded after LP failure");
    }

    // ============================================================
    // ATTACK 14: V3 LP - SWAP FAILS
    // ETH stays in contract for downstream - isolation model
    // DAI/staking must still execute correctly
    // ============================================================
    function test_Attack_V2LP_SwapFails() public {
        ...
        // No ETH stranded - downstream consumed it
        assertLt(address(plate).balance, 0.01 ether,
            "No ETH stranded after swap failure");
    ...
    // After routeETH() all ETH must be allocated - none stranded
    // ============================================================
    function test_Attack_V2LP_ZeroResidualBalances() public {
        router.setDAIReturn(1000 * 1e18);

        // Seed router for LP success path
        plate.transfer(address(router), 5_000_000 * 1e18);
        router.setPlateReturn(1_000_000 * 1e18);
        ...
        // Contract ETH balance must be near-zero
        assertLt(address(plate).balance, 0.01 ether,
