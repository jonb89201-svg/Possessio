...
contract PLATEAttackTest is Test {
    ...
    // PLATE flushed to Treasury - ETH stays (V3 isolation)
    // Only LP allocation remains in contract
    // ============================================================
    function test_Attack_V2LP_SwapSucceedsLPFails() public {
        ...
        // V3 isolation: only LP portion (25% = 1 ETH) may remain
        assertLe(address(plate).balance, 1.01 ether,
            "Only LP allocation ETH may remain after LP failure");
    }

    // ============================================================
    // ATTACK 14: V3 LP - SWAP FAILS
    // ETH stays in contract - only LP portion remains
    // ============================================================
    function test_Attack_V2LP_SwapFails() public {
        ...
        // V3 isolation: only LP portion (25% = 1 ETH) may remain
        assertLe(address(plate).balance, 1.01 ether,
            "Only LP allocation ETH may remain after swap failure");
    ...
    // ============================================================
    // ATTACK 15: V3 LP - ZERO RESIDUAL BALANCES (SUCCESS PATH)
    // When LP succeeds all ETH consumed
    // ============================================================
    function test_Attack_V2LP_ZeroResidualBalances() public {
        router.setDAIReturn(1000 * 1e18);

        // Seed router with enough PLATE to pass slippage check
        // ethForSwap = 10*25%*50% = 1.25 ETH
        // expectedPlate = 1.25 ETH / 1e-6 = 1,250,000 PLATE
        // minPlateOut = 1,250,000 * 90% = 1,125,000 PLATE
        plate.transfer(address(router), 10_000_000 * 1e18);
        router.setPlateReturn(2_000_000 * 1e18);

        vm.deal(address(plate), 10 ether);

        plate.routeETH();

        // LP succeeded - all ETH consumed
        assertLt(address(plate).balance, 0.01 ether,
