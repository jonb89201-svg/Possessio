...
contract PossessioPaymentsTest is Test {
    ...
    function setUp() public {
        ...
        //   cbETH/ETH (was ETH/USD): 18-decimal exchange-rate, 0.98e18 healthy peg
        //   DAI/ETH   (was DAI/USD): 8-decimal price-feed retained
        ...
        // v2.4.1 constructor signature: 14 params (was 11).
        // New: weth_, aeroRouter_, morphoVault_ inserted in canonical order.
    ...
    function test_Deploy_RevertsZeroOwner() public {
        ...
        new PossessioPayments(
            ...
            address(weth), address(router), address(aeroRouter), address(morphoVault),
            address(chainlinkEth), address(chainlinkDai),
            address(lstRates),
    ...
    function test_Deploy_RevertsZeroWETH() public {
        ...
        new PossessioPayments(
            MERCHANT,
            address(usdc), address(cbeth), address(dai),
            address(0), address(router), address(aeroRouter), address(morphoVault),
            address(chainlinkEth), address(chainlinkDai),
            address(lstRates),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    ...
    function test_Deploy_RevertsZeroAeroRouter() public {
        ...
        new PossessioPayments(
            MERCHANT,
            address(usdc), address(cbeth), address(dai),
            address(weth), address(router), address(0), address(morphoVault),
            address(chainlinkEth), address(chainlinkDai),
            address(lstRates),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    ...
    function test_Deploy_RevertsZeroMorphoVault() public {
        ...
        new PossessioPayments(
            MERCHANT,
            address(usdc), address(cbeth), address(dai),
            address(weth), address(router), address(aeroRouter), address(0),
            address(chainlinkEth), address(chainlinkDai),
            address(lstRates),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    ...
    function test_Deploy_RevertsOnWrongCbEthOracleDecimals() public {
        // v2.4.1 — Constructor decimal-class guard requires cbETH/ETH to be 18-decimal
        ...
        new PossessioPayments(
            ...
            address(usdc), address(cbeth), address(dai),
            address(weth), address(router), address(aeroRouter), address(morphoVault),
            address(wrongClass), address(chainlinkDai),
            address(lstRates),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    }
    ...
    function test_Deploy_RevertsOnWrongDaiOracleDecimals() public {
        // v2.4.1 — Constructor decimal-class guard requires DAI/ETH to be 8-decimal
        ...
        new PossessioPayments(
            ...
            address(usdc), address(cbeth), address(dai),
            address(weth), address(router), address(aeroRouter), address(morphoVault),
            address(chainlinkEth), address(wrongClass),
            address(lstRates),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    }
...
contract PossessioPaymentsAutomationTest is Test {
    ...
    function setUp() public {
        ...
        ethOracle   = new MockChainlink_A(18);  // v2.4.1 — cbETH/ETH 18-dec
        daiOracle   = new MockChainlink_A(8);   // v2.4.1 — DAI/ETH 8-dec
        lstRates    = new MockLSTRates_A();

        // v2.4.1 — Constructor signature: 14 params with weth/aeroRouter/morphoVault added
        payments = new PossessioPayments(
            OWNER,
            address(usdc),
            address(cbeth),
