 ...
 *        Phase 2.1 SSV Citadel dormant gating.
 *
 * STRATEGY: Inline mocks for USDC/DAI/cbETH/V3Router/Chainlink/LSTRates/SSVNetwork.
...
contract MockCbETH {
    ...
    /// @notice cbETH withdraw mock — burns cbETH, returns ETH to caller (Phase 2.1)
    function withdraw(uint256 amount) external {
        require(_balances[msg.sender] >= amount, "insuf");
        _balances[msg.sender] -= amount;
        // Mock returns ETH at 1.05 ETH per cbETH per LST rate
        uint256 ethOut = (amount * 1.05e18) / 1e18;
...
/**
 * @notice Mock SSV Network registry. Accepts validator registration calls
 *         and tracks them for test assertions. Models the ETH-Pay model
 *         (msg.value carries the validator deposit).
 */
contract MockSSVNetwork {
    ...
    uint64[] public lastOperatorIds;
    uint256 public lastEthReceived;
    uint256 public registrationCount;
    bool public _shouldRevert;

    function setShouldRevert(bool b) external { _shouldRevert = b; }
    ...
    function registerValidator(
        ...
        require(!_shouldRevert, "MockSSV: reverts");
        lastPubKey = publicKey;
        delete lastOperatorIds;
        for (uint256 i = 0; i < operatorIds.length; i++) {
            lastOperatorIds.push(operatorIds[i]);
        }
...
contract PossessioPaymentsTest is Test {
    ...
    function test_Deploy_ImmutablesSet() public {
        ...
        assertEq(payments.SSV_NETWORK(),            address(ssvNetwork));
    }
    ...
    function test_Deploy_RevertsZeroOwner() public {
        ...
        new PossessioPayments(
            ...
            address(lstRates), address(ssvNetwork),
    ...
    function test_Deploy_RevertsZeroSSVNetwork() public {
        vm.expectRevert(PossessioPayments.InvalidAddress.selector);
        new PossessioPayments(
            MERCHANT,
            address(usdc), address(cbeth), address(dai),
            address(router), address(chainlinkEth), address(chainlinkDai),
            address(lstRates), address(0),
            MIN_BATCH, DAI_CEILING, DAILY_LIMIT
        );
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Five new invariants added per ChatGPT/Gemini joint review post rETH-Base
    // verification finding. Codebyte Law: rETH on Base is bridged-not-native.
    // These invariants enforce the cbETH-only architecture at the test layer.
    ...
    function test_Invariant_cbETHExposure100Percent() public {
        ...
        uint256 gauge = payments.getCitadelProgress();
    ...
    function test_Invariant_NoUnreachableLSTBalances() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            // Defensive 2-day warp to clear cooldown unambiguously
    ...
    // cbETH-Invariant 5: UNSUPPORTED LST GUARD
    function test_Invariant_UnsupportedLSTErrorExists() public {
        bytes4 unsupportedSelector = PossessioPayments.UnsupportedLST.selector;
        assertTrue(unsupportedSelector != bytes4(0), "UnsupportedLST error defined");

        assertEq(address(payments.CBETH()), address(cbeth), "CBETH is the ratified LST");
    }

    // ═══════════════════════════════════════════════════════════════════════
    //                    PHASE 2.1 SSV CITADEL TESTS
    // ═══════════════════════════════════════════════════════════════════════

    function test_Citadel_InitialStateIsAccumulating() public {
        assertEq(uint256(payments.citadelState()), uint256(PossessioPayments.CitadelState.ACCUMULATING));
    }

    function test_Citadel_SSVNetworkAddressSet() public {
        assertEq(payments.SSV_NETWORK(), address(ssvNetwork));
    }

    function test_Citadel_ThresholdConstants() public {
        assertEq(payments.ELIGIBILITY_THRESHOLD(), 32.16 ether);
        assertEq(payments.VALIDATOR_DEPOSIT(),     32 ether);
        assertEq(payments.MAX_OPERATORS(),         13);
    }

    function test_Citadel_InitiateRevertsWhenThresholdNotReached() public {
        vm.expectRevert(PossessioPayments.CitadelNotReady.selector);
        vm.prank(MERCHANT);
        payments.initiateLSTRedemption();
    }

    function test_Citadel_AbortRevertsWhenThresholdNotReached() public {
        vm.expectRevert(PossessioPayments.CitadelNotReady.selector);
        vm.prank(MERCHANT);
        payments.abortActivation();
    }

    function test_Citadel_CompleteActivationRevertsWhenThresholdNotReached() public {
        bytes memory pubKey = hex"01";
        uint64[] memory ids = new uint64[](1);
        ids[0] = 1;
        bytes memory shares = hex"02";

        vm.expectRevert(PossessioPayments.CitadelNotReady.selector);
        vm.prank(MERCHANT);
        payments.completeActivation(pubKey, ids, shares, false);
    }

    function test_Citadel_GuardianCanAddVettedOperator() public {
        bytes32 gRole = payments.GUARDIAN_ROLE();
        vm.prank(MERCHANT);
        payments.grantRole(gRole, GUARDIAN);

        vm.prank(GUARDIAN);
        payments.addVettedOperator(42);

        assertTrue(payments.vettedOperators(42));
    }

    function test_Citadel_GuardianCanRemoveVettedOperator() public {
        bytes32 gRole = payments.GUARDIAN_ROLE();
        vm.prank(MERCHANT);
        payments.grantRole(gRole, GUARDIAN);
        ...
        payments.addVettedOperator(42);
        payments.removeVettedOperator(42);
        vm.stopPrank();

        assertFalse(payments.vettedOperators(42));
    }

    function test_Citadel_AttackerCannotAddVettedOperator() public {
        vm.expectRevert();
        vm.prank(ATTACKER);
        payments.addVettedOperator(42);
    }

    function test_Citadel_OwnerCannotAddVettedOperatorDirectly() public {
        vm.expectRevert();
        vm.prank(MERCHANT);
        payments.addVettedOperator(42);
    }

    function test_Citadel_ValidatorPubKeyEmptyInitially() public {
        bytes memory empty = payments.validatorPubKey();
        assertEq(empty.length, 0);
    }

    function test_Citadel_ThresholdReachedFalseInitially() public {
        assertFalse(payments.thresholdReached());
    }
