...
// If the repo later adopts shared mocks, lift these contracts to
// test/mocks/ and restore imports — no test logic changes required.
...
contract POSSESSIO_v2_Invariants_t is Test {
    ...
    function _seedSAV(uint256 amount) internal {
        ...
        // forge-lint: disable-next-line erc20-unchecked-transfer — Test fixture seeding helper; STEEL ERC20 reverts on failure; downstream savDeposit asserts state.
...
// ═══════════════════════════════════════════════════════════════════════════
//
// These mocks are minimal implementations of the external dependencies
// required to instantiate PossessioHook in setUp(). Each mock implements
// only the interface surface actually exercised by the test or required by
// the hook constructor's address validation.
//
// Interface sources (from POSSESSIO_v2.sol):
//   - IChainlinkFeed        — line 87
//   - IcbETH                — line 93
//   - IWETH                 — line 99
//   - IV3SwapRouter         — line 73
//   - IPoolManager          — imported from v4-core
//
// Design principles:
//   - No state beyond what the test requires
//   - No reverts unless the test specifically exercises a failure path
//   - balanceOf returns real internal accounting (so cbeth.mint affects
//     hook.balanceOf checks correctly in invariant assertions)
// ═══════════════════════════════════════════════════════════════════════════

/// @notice Minimal Chainlink AggregatorV3 mock. Stores a single round.
///         Test seeds round data via setRoundData() in setUp().
...
/// @notice Minimal cbETH mock. Implements IcbETH (deposit / withdraw /
///         balanceOf) plus mint() for test setup convenience.
///         deposit() mints 1:1 against ETH sent. withdraw() burns and
///         returns ETH 1:1 (the hook handles real exchange rate logic
///         elsewhere; the mock just preserves accounting symmetry).
contract MockcbETH {
    ...
    /// @dev Test helper — mint cbETH to any address without going through
    ///      deposit(). Used by invariant tests to inject "phantom" balance
    ///      and verify cbETHPrincipal does not drift.
...
/// @notice Minimal WETH mock. Implements IWETH surface. Deposit/withdraw
///         are 1:1 ETH↔WETH. The test does not exercise WETH paths, but
///         the hook constructor validates non-zero address.
contract MockWETH {
    mapping(address => uint256) private _balances;
...
/// @notice Minimal V3 router mock. Implements exactInputSingle by returning
///         amountOutMinimum. Sufficient to satisfy the hook's swap call
///         signature without simulating real AMM math. The invariant tests
///         do not exercise the swap path; this mock exists only so the
///         constructor passes address-validation.
contract MockV3Router {
    ...
    {
        // Return minOut so the hook's slippage check passes deterministically.
        // No actual token movement — the invariant suite does not exercise
...
/// @notice Minimal IPoolManager mock. The constructor only validates that
///         the address is non-zero; no method calls are exercised by the
///         invariant suite. A bare contract satisfies that requirement.
contract MockPoolManager {
    // Empty by design. Expand only if a future test exercises a specific
    // PoolManager method, in which case add the minimal surface.
}

/// @notice Minimal ERC20 mock. Standard surface for DAI in test setup.
///         Constructor takes (name, symbol, decimals) per existing test
///         convention. No mint restrictions — tests can mint freely.
contract MockERC20 {
    string  public name;
