...
- **Not yet read this pass (follow-on):** `PLATE`/`PLATEStaking`, `PossessioWhiskyMarket`, `LSTExchangeRate`, `SymmetryGuardCore` (base of x402Core — its DustSpam/RoundTrip guard read at the interface level via `totalFeeBps`/`_observe`, not yet fully audited), `PossessioCouncilToken`, `HandshakeLib`, `PossessioAutoTarget` fork behavior, `PossessioTestnetLaunchPool`. ~5,250 LOC.
