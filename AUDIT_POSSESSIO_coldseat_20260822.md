...
| `PossessioSaltPool` | 195 | sound; pull factory-only, refill keeper-only + compute-only (no value path), keeper-separation enforced; **G-1 FIXED** — refill now enforces the factory-permissioned salt prefix on-chain |
...
| `PossessioLaunchFactory` | 410 | PossessioFactory pattern + v4 pairing; owner un-spoofable, atomic, codehash-pinned; **L-1 (MEASURED Medium, pool-init front-run DoS) FIXED** via BEFORE_INITIALIZE-flag guard |
...
- **FIXED in this branch (MEASURED red→green).** `deployLaunch` now requires the launch address to carry the v4 **`BEFORE_INITIALIZE` flag** (`if (uint160(launch) & (1<<13) == 0) revert LaunchHookNotInitGated(launch);`, step 7a). With the flag set, v4's `initialize` invokes `launch.beforeInitialize` on *every* init of the key — so an attacker's pre-init against the still-codeless launch address hits a hook call to a code-less address and reverts (`InvalidHookResponse` in v4); only the factory's own init, *after* the launch is deployed and its `beforeInitialize` returns the expected selector, succeeds. The guard fails closed on a mis-mined salt, so no un-gated pool can ever be minted. Regression `test/PossessioLaunchFactory_L1PairInitFrontrun.t.sol` (rewritten, 3 tests, green): `test_L1_unflaggedAddress_wouldBeFrontRunnable` shows an unflagged address IS front-runnable (the pre-fix world, via a v4-faithful PoolManager that calls `beforeInitialize` and reverts per-key on already-init); `test_L1fix_AttackerCannotPreInit_LegitStillWorks` shows the flagged address blocks the attacker while the honest deploy works; `test_L1fix_factoryRejectsUnflaggedSalt` shows the guard refuses an unflagged salt (fee refunded, salt unconsumed).
- **Two residual requirements for the launch rail (documented, keeper/template-side — do NOT deploy the LaunchFactory until both hold):** (1) the keeper must mine salts so the launch address carries `BEFORE_INITIALIZE` (`0x8C8 | 0x2000`) — else `deployLaunch` fails closed; (2) the (out-of-repo) launch template MUST implement `beforeInitialize` returning its selector, else even the legit init reverts (fail-closed, safe). Provenance: **DERIVED** fix on a **MEASURED** finding, MEASURED closed.
...
- **FIXED in this branch.** `PossessioSaltPool.refillSalts` now enforces `address(bytes20(salt)) == factory` per salt (`revert SaltNotFactoryPermissioned(salt)` otherwise), converting "keeper trusted to mine permissioned salts" into "contract verifies it" — a keeper mistake can no longer seed a front-runnable (permissionless-prefix) salt. Tests `test_G1_refill_nonFactoryPrefixed_reverts` / `test_G1_refill_factoryPrefixed_accepted` (green). Provenance: **DERIVED** (CreateX `_guard` semantics + SaltPool source), fix verified in terminal.
...
- **Confirmed DORMANT / out of live scope (MEASURED — no live dependency):** `PLATE.sol` and `PLATEStaking.sol` are V1 relics — **no `src/` contract imports either** (the five "PLATE" mentions across L1Anchor/hook/factory/SAV are all lineage *comments*), and the V2 hook's own header states it: *"STAKING REMOVED: PLATEStaking was ceremony over burn — eliminated from v2"* (`POSSESSIO_v2-6-3.sol:219`). The live token is STEEL — `ERC20("PLATE","STEEL")` embedded in the V2 hook, already audited sound this pass. Architect confirmed PLATE is unused. Excluded from the audit as dead code.
- **Not audited (non-core / non-mainnet, lower EV):** `PossessioWhiskyMarket` (niche market), `PossessioAutoTarget` fork behavior, `PossessioTestnetLaunchPool` (testnet-only). ~1,900 LOC.
