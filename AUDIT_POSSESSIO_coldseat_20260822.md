...
- **Not yet read this pass (follow-on):** `PossessioPool` (473 — partially read: `receiveInfraFunds`/`isInfraSink` verified for the x402 surplus-routing invariant; velocity-floor + rest not yet fully audited), `PossessioLaunchFactory` (410), `PLATE`/`PLATEStaking`, `PossessioWhiskyMarket`, `LSTExchangeRate`, `SymmetryGuardCore` (base of x402Core — its DustSpam/RoundTrip guard read at the interface level via `totalFeeBps`/`_observe`, not yet fully audited), `PossessioCouncilToken`, `HandshakeLib`, `PossessioAutoTarget` fork behavior, `PossessioTestnetLaunchPool`. ~6,100 LOC.
