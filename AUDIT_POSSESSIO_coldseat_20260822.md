...
- **Not yet read this pass (follow-on):** the V3 launch **template** itself (the contract `PossessioLaunchFactory` deploys — carries the owner's 2% fee engine and is the L-1 fix's `beforeInitialize` home; NOT yet located/audited), `PLATE`/`PLATEStaking`, `PossessioWhiskyMarket`, `LSTExchangeRate`, `SymmetryGuardCore` (base of x402Core — its DustSpam/RoundTrip guard read at the interface level via `totalFeeBps`/`_observe`, not yet fully audited), `PossessioCouncilToken`, `HandshakeLib`, `PossessioAutoTarget` fork behavior, `PossessioTestnetLaunchPool`. ~5,250 LOC.
