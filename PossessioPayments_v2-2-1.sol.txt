...
/**
 * @dev v2.2 (2026-05-12) adds Chainlink Automation for sweep().
...
contract PossessioPayments is AccessControl, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    /// @notice Gates internal-only external entrypoints (dispatcher) to self-calls.
    modifier onlySelf() {
     ...
     *
     *         performData carries the operator's minDaiOut and minCbEthOut
     *         slippage guards as ABI-encoded (uint256, uint256). These are
     *         passed through to sweep(). The values are set by the operator
     *         via setSweepSlippageGuards() — see below.
     */
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        return (true, abi.encode(sweepSlippageMinDai, sweepSlippageMinCbEth));
    }
        ...
        nonReentrant
    {
        ...
        (uint256 minDaiOut, uint256 minCbEthOut) = abi.decode(performData, (uint256, uint256));
        ...
        this.dispatchSweep(minDaiOut, minCbEthOut);
    ...
    /// @notice External-only dispatcher from performUpkeep into sweep().
    ///         this.sweep(...) sets msg.sender to address(this).
    ///         sweep() is gated on OWNER_ROLE || OPERATOR_ROLE — neither role
    ///         is held by address(this), so the OWNER must grant OPERATOR_ROLE
    ///         to the contract itself before registration, OR the dispatcher
    ///         needs an alternate entrypoint. See note in audit comments.
    function dispatchSweep(uint256 minDaiOut, uint256 minCbEthOut) external onlySelf {
        this.sweep(minDaiOut, minCbEthOut);
    }
    ...
    function setSweepSlippageGuards(uint256 minDai, uint256 minCbEth)
        external
        onlyRole(OWNER_ROLE)
    {
