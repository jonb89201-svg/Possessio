...
contract MockV3Router_A {
    ...
    // (0x04e45aaf) matches what PossessioPayments_v2-2.sol's sweep path
    // actually calls. Previous stub used `bytes calldata` which gave a
    // different selector and reverted with "unrecognized function selector"
    // when the sweep tried to swap. Struct mirrors the main MockV3Router
    // for substrate consistency.
    struct ExactInputSingleParams {
        address tokenIn;
        address tokenOut;
        uint24  fee;
