 ...
 * ╚═╝     ╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝
 *
 * POSSESSIO PROTOCOL — v2.4 (STEEL + Chainlink Automation)
 * Protocol Liquidity Asset Treasury Engine, rebuilt as Uniswap V4 hook
 *
 * v2.4 adds Chainlink Automation v2.3 for routeETH() + harvestRewards()
...
contract PossessioHook is IUnlockCallback, ReentrancyGuard, Ownable2Step, AutomationCompatibleInterface {
    ...
    function routeETH() external nonReentrant notPaused {
        ...
        // X-LINK: mint a monotonic one-time secret and arm immediately before the
        // internal call. _routeETH verifies + consumes as its first action.
    ...
    function _routeETH(uint256 secret) internal {
        ...
        // exemption below covers all non-reward callers via msg.sender checks; the
    ...
    function _swapETHToDAI(uint256 ethAmt) internal {
        ...
        try CHAINLINK_DAI_ETH.latestRoundData() returns (
            ...
            // Oracle staleness threshold is 3600s; validator's ~12s manipulation window is immaterial vs that bound.
            ...
            if (valid) {
                // daiEthPrice > 0 validated on line above; positive int256 always fits in uint256.
    ...
    /**
     * @notice Deploy ETH to cbETH (rewards-accruing treasury asset).
    ...
    function _deployToStaking(uint256 ethAmt) internal {
        ...
        uint256 cbAmt = cbETHPaused ? 0 : ethAmt;
        ...
        emit StakingDeployed(cbAmt, block.timestamp);
    }
    ...
    function harvestRewards() external nonReentrant notPaused {
        // X-LINK: mint a monotonic one-time secret and arm immediately before the
        // internal call. _harvestRewards verifies + consumes as its first action.
        uint256 secret = ++_executionNonce;
        _activeExecutionSecret = secret;
    ...
    function _harvestRewards(uint256 secret) internal {
        ...
        if (cbBal > cbETHPrincipal) {
            uint256 rewardsLST = cbBal - cbETHPrincipal;
            uint256 ethBefore  = address(this).balance;
            try cbETH.withdraw(rewardsLST) {
        ...
        nonReentrant
    {
        ...
        if (task == UpkeepTask.ROUTE_ETH) {
            ...
            // X-LINK: mint a monotonic one-time secret and arm immediately before
            // the internal call. _routeETH verifies + consumes. onlyForwarder +
            // nonReentrant + upkeepPaused + replay protection remain this path's
            // authorization; X-LINK is the additional proof that _routeETH was
            // reached through this sanctioned branch.
            uint256 secret = ++_executionNonce;
            _activeExecutionSecret = secret;
            _routeETH(secret);
            ...
            // X-LINK: same mint-arm-call pattern for the harvest branch.
            uint256 secret = ++_executionNonce;
            _activeExecutionSecret = secret;
            _harvestRewards(secret);
    ...
    function _checkDepeg() internal {
        try CHAINLINK_CBETH_ETH.latestRoundData() returns (
            ...
            // Oracle staleness check at 3600s threshold; validator's ~12s manipulation window is immaterial.
            // forge-lint: disable-next-line(block-timestamp)
            ...
            if (answer < DEPEG_THRESH && !cbETHPaused) {
                cbETHPaused = true;
                // answer ∈ (0, DEPEG_THRESH=97e6) validated above; (1e8 - answer) ∈ (3e6, 1e8) is always positive, fits uint256.
                // forge-lint: disable-next-line(unsafe-typecast)
